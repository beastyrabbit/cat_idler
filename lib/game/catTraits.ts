...
export function summarizeCatTraits(
	spriteParams: Record<string, unknown> | null | undefined,
): CatTraitSummary {
	const p = (spriteParams ?? {}) as Record<string, unknown>;

	const lineage =
		typeof p.peltName === "string" && p.peltName.trim()
			? p.peltName
			: "Unknown lineage";

	const colour =
		typeof p.colour === "string" && p.colour.trim() ? p.colour : "Unknown coat";
	const tint =
		typeof p.tint === "string" && p.tint !== "none" ? ` · ${p.tint}` : "";
	const coat = `${colour}${tint}`;

	const eye1 =
		typeof p.eyeColour === "string" && p.eyeColour.trim()
			? p.eyeColour
			: "Unknown eyes";
	const eye2 =
		typeof p.eyeColour2 === "string" && p.eyeColour2.trim()
			? p.eyeColour2
			: null;
...
export function summarizeCatIdentity(
	spriteParams: Record<string, unknown> | null | undefined,
): CatIdentitySummary {
	const p = (spriteParams ?? {}) as Record<string, unknown>;
	...
	const accessoriesRaw = Array.isArray(p.accessories)
		? p.accessories.filter((item) => typeof item === "string")
		: [];
	const scarsRaw = Array.isArray(p.scars)
		? p.scars.filter((item) => typeof item === "string")
		: [];
