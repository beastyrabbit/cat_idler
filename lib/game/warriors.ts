 ...
 * fight well; hunters can fight badly; every other cat stays out of the
 * fight. Combat power blends a cat's raw stats, its warrior trade level, the
 * gear it draws from the colony stockpile, and the upgrade-tree combat
 * modifiers (weaponsmithing / armorsmithing).
 *
...
/** Combat effectiveness of a hunter pressed into the fight (badly). */
...
/** How much a role contributes to the fight. 0 means "won't be mustered". */
...
/** True when a cat's specialization lets it join the muster at all. */
 ...
 * (defense + armor) scaled by defenseMult, weighted by its role factor and
 * lifted by its warrior trade level. Non-combatants score 0.
 */
...
/**
 * Muster the colony's defenders against a raid. Every warrior and hunter
 * turns out; the available weapons and armor are handed to the strongest
 * warriors first (then hunters), so scarce gear arms the cats who use it
 * best. Returns the total power and exactly how much gear was consumed.
 */
