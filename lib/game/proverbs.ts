...
export function getResourceProverb(
	...
	// Pick a critical resource list using the seed for variety
	const first = rollSeeded(seed);
	const { value, nextSeed } = rollSeeded(first.nextSeed * 7919);
	const listIndex = Math.floor(value * criticalResources.length);
	const selectedList = criticalResources[listIndex];
	const { value: proverbValue } = rollSeeded(nextSeed);
	const index = Math.floor(proverbValue * selectedList.length);
