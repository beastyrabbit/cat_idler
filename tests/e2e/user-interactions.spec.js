	...
	try {
		...
		if (createInput.length > 0 && (await createInput[0].isDisplayed())) {
			...
			const leaderModal = await driver.findElements(
				By.xpath(
					"//*[contains(text(), 'Leader') or contains(text(), 'leader')]",
				),
			);
			if (leaderModal.length > 0) {
				await driver.sleep(1000);
				const candidates = await driver.findElements(
					By.css('button, [role="button"], [class*="border"]'),
				);
		...
		const feedButtons = await driver.findElements(
			By.xpath(
				"//button[contains(text(), 'Food') or contains(text(), 'food') or contains(text(), 'Feed')]",
			),
		);
		...
		const waterButtons = await driver.findElements(
			By.xpath(
				"//button[contains(text(), 'Water') or contains(text(), 'water')]",
			),
		);
		...
		const worldMapButtons = await driver.findElements(
			By.xpath(
				"//button[contains(text(), 'World') or contains(text(), 'world')]",
			),
		);
		if (worldMapButtons.length > 0) {
			...
			const colonyButtons = await driver.findElements(
				By.xpath(
					"//button[contains(text(), 'Colony') or contains(text(), 'colony')]",
				),
			);
		...
		const taskQueue = await driver.findElements(
			By.xpath("//*[contains(text(), 'Task') or contains(text(), 'task')]"),
		);
		...
		const eventLog = await driver.findElements(
			By.xpath("//*[contains(text(), 'Event') or contains(text(), 'event')]"),
		);
