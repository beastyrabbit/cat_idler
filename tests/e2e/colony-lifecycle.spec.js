...
export default async function testColonyLifecycle(headed = false, port = 3000) {
	...
	try {
		...
		if (
			pageSource.includes("View Colony") ||
			pageSource.includes("Active colony")
		) {
			...
			try {
				...
				if (viewLinks.length > 0) {
					...
					if (currentUrl.includes("/colony/")) {
						...
						const hasFood =
							pageSource2.includes("Food") || pageSource2.includes("food");
						const hasWater =
							pageSource2.includes("Water") || pageSource2.includes("water");
						if (hasFood || hasWater) {
				...
				console.log(
					"  Note: Could not navigate to existing colony, will try to create new one",
				);
			}
		}
		...
		if (inputs.length > 0) {
			...
			try {
				const buttons = await driver.findElements(
					By.xpath("//button[contains(text(), 'Create Colony')]"),
				);
				if (buttons.length > 0) {
			...
			if (
				currentPageSource.includes("Food") ||
				currentPageSource.includes("Cats")
			) {
		...
		if (!currentUrl.includes("/colony/")) {
			const viewLinks = await driver.findElements(
				By.xpath("//a[contains(text(), 'View') or contains(text(), 'Colony')]"),
			);
			if (viewLinks.length > 0) {
				await viewLinks[0].click();
				await driver.sleep(3000);
		...
		const hasResources =
			finalPageSource.includes("Food") ||
			finalPageSource.includes("Water") ||
			finalPageSource.includes("Herbs");
		...
		const pageSource2 = await driver.getPageSource();
		const hasLeaderModal =
			pageSource2.includes("Select") && pageSource2.includes("Leader");
		if (hasLeaderModal) {
			...
			try {
				const candidates = await driver.findElements(
					By.xpath(
						"//div[contains(@class, 'border')] | //button | //div[@role='button']",
					),
				);
				if (candidates.length > 0) {
					...
					// Click select button
					const selectButtons = await driver.findElements(
		...
		let hasFood =
			resourceText.includes("Food") || resourceText.includes("food");
		let hasWater =
			resourceText.includes("Water") || resourceText.includes("water");
		...
		if (!hasFood && !hasWater) {
			...
			console.log(
				"  ⚠ Resource bars not immediately visible (page may still be loading)",
			);
		} else {
