...
describe("idle rules", () => {
	it("detects strategic conflicts correctly", () => {
		expect(
			hasConflictingStrategicJob("leader_plan_hunt", [
				{ kind: "hunt_expedition" },
			]),
		).toBe(true);
		expect(
			hasConflictingStrategicJob("leader_plan_house", [
				{ kind: "build_house" },
			]),
		).toBe(true);
		expect(hasConflictingStrategicJob("ritual", [{ kind: "ritual" }])).toBe(
			true,
		);
		expect(
			hasConflictingStrategicJob("leader_plan_hunt", [{ kind: "supply_food" }]),
		).toBe(false);
		expect(
			hasConflictingStrategicJob("supply_food", [{ kind: "ritual" }]),
		).toBe(false);
	});
	...
	it("starts ritual only when requested, stable, and conflict-free", () => {
		...
		expect(shouldStartRitual(Date.now(), { food: 15, water: 20 }, [])).toBe(
			false,
		);
		expect(
			shouldStartRitual(Date.now(), { food: 20, water: 20 }, [
				{ kind: "ritual" },
			]),
		).toBe(false);
		expect(shouldStartRitual(Date.now(), { food: 20, water: 20 }, [])).toBe(
			true,
		);
	});
	...
	it("computes consumption with resilience impact", () => {
		...
		const resilient = consumptionForTick(5, 600, {
			...baseUpgrades,
			resilience: 4,
		});
	...
	it("maps supply levels to colony status bands", () => {
		expect(nextColonyStatus({ food: 1, water: 2, herbs: 3 })).toBe(
			"struggling",
		);
		expect(nextColonyStatus({ food: 30, water: 30, herbs: 20 })).toBe(
			"thriving",
		);
		expect(nextColonyStatus({ food: 10, water: 10, herbs: 10 })).toBe(
			"starting",
		);
	});
