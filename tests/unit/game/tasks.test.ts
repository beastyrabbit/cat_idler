...
describe("getOptimalCatForTask", () => {
	it("should pick cat with highest hunting for hunt task", () => {
		const cats = [
			createMockAdultCat({
				name: "LowHunter",
				stats: { ...createMockAdultCat().stats, hunting: 30 },
			}),
			createMockHunterCat({
				name: "HighHunter",
				birthTime: Date.now() - 30 * 60 * 60 * 1000,
			}),
			createMockAdultCat({
				name: "Other",
				stats: { ...createMockAdultCat().stats, hunting: 20 },
			}),
		];
	...
	it("should pick cat with highest medicine for heal task", () => {
		const cats = [
			createMockAdultCat({
				name: "LowHealer",
				stats: { ...createMockAdultCat().stats, medicine: 30 },
			}),
			createMockHealerCat({
				name: "HighHealer",
				birthTime: Date.now() - 30 * 60 * 60 * 1000,
...
describe("getAssignedCat", () => {
	...
	const cats = [
		...
		createMockAdultCat({
			name: "Builder",
			stats: { ...createMockAdultCat().stats, building: 80 },
			birthTime: baseTime,
		}),
	];
