...
| P12 | Sim expansion: skills, officers, spatial stockpiles, workshop chains | in progress — seven specialist manual/officer domains, a narrow founding Leader Hunt/FetchWater/Scout safety floor, all 19 maintained skill gain/effect/UI paths, role gates, active physical shrine offerings, useful tools/costs, finite storage, physical and player-wire-confidential Accountant rounds, physical Mill/Sawmill/Wood Cutter/Stone Prep/Woodworking/Workshop/Smelter/Tannery routes, Steward-managed local reserves, eleven data-owned station recipe descriptors with eight enforced production gates and three founding-available baselines, physical farming, tree felling/hauling/replanting/regrowth, fishing, and the P19 raw Stone/Bone/source-cargo migration are verified; two aggregate benches, finite equipment authority, and 93 generated recipe IDs remain in progress |
| P13 | Client UI for P12: stockpile designation, officer assignment | in progress — designation/assignment, signed manual orders, exact building/farm/gather/road/governance controls, authoritative election timing, durable per-cat typed labor preferences, all eight physical processors' editable add/remove/reorder/repeat/pause queues and truthful inspectors (including the exact missing study for a locked recipe), Steward pile provenance/route visibility, the purchasable 500-study ledger, crop/timber HUD state, visible farm stages, and distinct open station compositions shipped; extend the generic queue UI as broader physical recipes land |
...
| P19 | Item/material economy: crafting chains, traders, coin | in progress — the canonical source/station/taxonomy contract now reconciles P12 and P16 while preserving stable IDs and open-top buildings; raw Stone and Bone, physical quarry Stone/rubble/Ore and hunt Hide/Bone cargo, finite fresh Fish, physical scaffold inputs, eleven station recipe descriptors/default queues, physical Woodworking Tools, useful tools, trade goods, a shrine-visiting physical trader with finite persisted stock/purse/wagon, coin, and stable weighted item units with wear/break/repair are live. Three remaining aggregate station conversions, finite functional-equipment authority, recipe/material breadth, and broader local inventories remain |
...
- **P12.4a/b workshop chains + Accountant direction** — workshop crafting covers
  ...
  Ore, and hunt Hide are carried cargo. Stone Prep and Woodworking are physical; next convert
  Clothier and Smithy to one-worker physical queues; then
...
- **P12.6 logistics** — general/limited stockpile designation, signed manual shrine orders, and
  ...
  seeded finite general storehouse and complete physical Mill/Sawmill/Wood Cutter/Stone Prep/Woodworking/Workshop/Smelter routes are
  ...
  blocked recovery persists without overfill. Other production chains still need the same contract.
...
- A full-page 500-study ledger with dependencies, filter/search/pan/zoom. Every study supports
  ...
  ownership; Woodworking now executes its founding recipe physically while three compatibility
  benches await conversion. Ninety-three generated recipes and 64 resources remain open.
...
- Visible hauling now uses the same semantic authority: every one of the 21 `CarryingKind` values
...
- Remaining breadth includes physical Clothier and Smithy queues; Bone item variants plus
...
- Physical-economy follow-ups remain explicit: stations other than Mill/Sawmill/Wood Cutter/Stone Prep/Woodworking/Workshop/Smelter
  need the station-local carry/work/delivery contract; climate resource hints must become real
...
| P19.C2 — remaining physical stations | in progress | C2.0 gives all six benches stable data-owned recipe descriptors, canonical resource sets, deterministic default queues, exact selected-recipe catalog/block metadata, rules-v0 grandfathering, and generic signed/persisted queue state. The Wood Cutter, Stone Prep, and Woodworking baseline recipes are descriptor-marked founding-available; their studies gate later recipes. C2.1 completes Wood Cutter's finite five-Logs→one-Plank route. C2.2 completes Stone Prep: five Stone travel to local input, one Process worker remains assigned across fetch/work/output and advances the selected 600-second `stone_to_blocks` batch, and one local Block travels to finite storage before aggregate credit. C2.3 completes Woodworking: one Craft worker sequentially fetches two Planks and two Blocks, consumes both local inputs atomically in one selected 600-second batch, and carries one whole scalar Tool from local output to finite storage before aggregate credit. C2.4 completes Tannery: five physically hunted Hide travel through inbound and local input, one Textile worker advances the selected 600-second `hide_to_leather` batch, and one Leather travels through local output and outbound cargo before finite-store credit. Independent construction reserves, comfort release, whole-unit capacity, queue controls, full storage, death/removal, exact restart, v2→v3/v3→v4 no-seed queue preservation, bit-frozen legacy timers, stable 1s/5s/60s projections, signed/passive guidance, exact player-Hunt attribution, HMAC bearer reconnect/restart, and inspector truth are regression-covered. The accepted client-owned exact 1024×768 `/tmp/physical-tannery-c2.png` (SHA-256 `260915ad91a1c8a69302a1508590cb1abe685831b43a686bd7cd44581a9502ff`) shows the selected open-top Tannery, Moss and the semantic Leather carrier glyph, 40% progress, inbound/local Hide 5, local/outbound Leather 1, the repeating queue, and every queue control without clipping or overlap. Finite Tool identity remains C3. Convert Clothier and Smithy through the same reusable physical-recipe seam. |
