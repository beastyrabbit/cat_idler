...
| P12 | Sim expansion: skills, officers, spatial stockpiles, workshop chains | in progress — seven specialist manual/officer domains, a narrow founding Leader Hunt/FetchWater/Scout safety floor, all 19 maintained skill gain/effect/UI paths, role gates, active physical shrine offerings, useful tools/costs, finite storage, physical and player-wire-confidential Accountant rounds, physical Mill/Sawmill/Workshop/Smelter routes, Steward-managed local reserves, eleven data-owned station recipe descriptors with eight enforced production gates and three behavior-neutral compatibility entries, physical farming, tree felling/hauling, fishing, and the P19 raw Stone/source-cargo migration are verified; Forester replanting is absent, while six aggregate benches, finite equipment authority, and 93 generated recipe IDs remain in progress |
...
- **P12.4a/b workshop chains + Accountant direction** — workshop crafting covers
  ...
  Ore, and hunt Hide are carried cargo. Next convert Wood Cutter, Stone Prep, Woodworking, Clothier,
  Tannery, and Smithy to one-worker physical queues; then
...
- **P12.6 logistics** — general/limited stockpile designation, signed manual shrine orders, and
  ...
  seeded finite general storehouse and complete physical Mill/Sawmill/Workshop/Smelter routes are
...
- A full-page 500-study ledger with dependencies, filter/search/pan/zoom. Every study supports
  ...
  false founding/non-runtime job claims are removed. Logging currently leaves permanent stumps;
  the vision's Forester replanting responsibility remains open. Research labor/building automation and
...
- Remaining breadth includes physical Wood Cutter, Stone Prep, Woodworking, Clothier, Tannery,
  and Smithy queues; Bone item variants plus finite gem/clay/metal raw stacks and variants; finished functional
...
| P19.C2 — remaining physical stations | in progress | C2.0 gives Wood Cutter, Stone Prep, Woodworking, Clothier, Tannery, and Smithy stable data-owned recipe descriptors, canonical resource sets (including raw Stone→Blocks), deterministic default queues, exact selected-recipe catalog/block metadata, rules-v0 grandfathering, and generic signed/persisted queue state. `aggregate_timer_compatibility` and a six-bench twin prove this scaffold does not yet alter production. Still convert the six benches to local input/output/transit state and one-worker/one-selected-recipe execution; death, removal, full storage, restart, signed controls, and inspectors must conserve and expose every unit. |
