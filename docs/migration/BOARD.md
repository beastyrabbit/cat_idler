...
| P12 | Sim expansion: skills, officers, spatial stockpiles, workshop chains | in progress — seven specialist manual/officer domains, a narrow founding Leader Hunt/FetchWater/Scout safety floor, all 19 maintained skill gain/effect/UI paths, role gates, active physical shrine offerings, useful tools/costs, finite storage, physical and player-wire-confidential Accountant rounds, all ten physical processor routes, Steward-managed local reserves, eleven data-owned runtime recipe descriptors with eight research-gated recipes and three founding baselines, physical farming/Fibre forage, tree felling/hauling/replanting/regrowth, fishing, and the P19 raw Stone/Bone/source-cargo migration are verified; finite equipment authority and 93 generated recipe IDs remain in progress |
| P13 | Client UI for P12: stockpile designation, officer assignment | in progress — designation/assignment, signed manual orders, exact building/farm/gather/road/governance controls, authoritative election timing, durable per-cat typed labor preferences, all ten physical processors' editable add/remove/reorder/repeat/pause queues and truthful inspectors (including the exact missing study for a locked recipe), Steward pile provenance/route visibility, the purchasable 500-study ledger, crop/timber HUD state, visible farm stages, and distinct open station compositions shipped; extend the generic queue UI as broader physical recipes land |
...
| P19 | Item/material economy: crafting chains, traders, coin | in progress — the canonical source/station/taxonomy contract now reconciles P12 and P16 while preserving stable IDs and open-top buildings; raw Stone and Bone, physical quarry Stone/rubble/Ore, hunt Hide/Bone, forage Fibre cargo, finite fresh Fish, physical scaffold inputs, eleven station recipe descriptors/default queues across all ten physical processors, useful tools, trade goods, a shrine-visiting physical trader with finite persisted stock/purse/wagon, coin, and stable weighted item units with wear/break/repair are live. Finite functional-equipment authority, recipe/material breadth, and broader local inventories remain |
...
- **P12.4a/b workshop chains + Accountant direction** — workshop crafting covers
  ...
  and Smithy are physical; next migrate functional equipment toward finite-item authority without double-counting compatibility
...
- A full-page 500-study ledger with dependencies, filter/search/pan/zoom. Every study supports
  ...
  ownership. Eight recipes are research-gated and three are founding baselines; all eleven execute
  physically. Ninety-three generated recipes and 64 resources remain open.
...
- Remaining breadth includes Bone item variants plus finite gem/clay/metal raw stacks and
  variants; finished functional
