...
- **P12.4a/b workshop chains + Accountant direction** — workshop crafting covers
  ...
  Ore, and hunt Hide are carried cargo. Next convert Stone Prep, Woodworking, Clothier, Tannery,
...
- **P12.6 logistics** — general/limited stockpile designation, signed manual shrine orders, and
  ...
  seeded finite general storehouse and complete physical Mill/Sawmill/Wood Cutter/Workshop/Smelter routes are
...
- Remaining breadth includes physical Stone Prep, Woodworking, Clothier, Tannery, and Smithy
...
- Physical-economy follow-ups remain explicit: stations other than Mill/Sawmill/Wood Cutter/Workshop/Smelter
...
| P19.C2 — remaining physical stations | in progress | C2.0 gives all six benches stable data-owned recipe descriptors, canonical resource sets, deterministic default queues, exact selected-recipe catalog/block metadata, rules-v0 grandfathering, and generic signed/persisted queue state. The Wood Cutter, Stone Prep, and Woodworking baseline recipes are descriptor-marked founding-available; their studies gate later recipes. C2.1 completes Wood Cutter: five Logs travel to local input, one Process worker remains assigned across fetch/work/output and advances the selected 600-second `logs_to_planks` batch, one local Plank travels to finite storage, and aggregate credit waits for delivery. Empty/reordered/repeat/pause queues, full storage, death/removal, exact restart, one-time pre-column queue seeding followed by durable player-empty state, stable full-route/XP projections at 1s/5s/60s, Forester passive viability, signed HMAC player guidance/restart, and inspector truth are regression-covered. The accepted exact 1024×768 own-framebuffer `/tmp/physical-wood-cutter-1024.png` visibly proves the selected queue/local/transit state, all controls, nonoverlapping minimap, and semantic Planks cargo. The final four-crate gate passed all 1,460 tests (one intentionally skipped) with strict all-target Clippy. Convert Stone Prep, Woodworking, Clothier, Tannery, and Smithy through the same reusable physical-recipe seam. |
