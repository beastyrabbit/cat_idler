...
| P12 | Sim expansion: skills, officers, spatial stockpiles, workshop chains | in progress — seven specialist manual/officer domains, a narrow founding Leader Hunt/FetchWater/Scout safety floor, all 19 maintained skill gain/effect/UI paths, role gates, active physical shrine offerings, useful tools/costs, finite storage, physical and player-wire-confidential Accountant rounds, nine physical processor routes through Clothier, Steward-managed local reserves, eleven data-owned runtime recipe descriptors with eight research-gated recipes and three founding baselines, physical farming/Fibre forage, tree felling/hauling/replanting/regrowth, fishing, and the P19 raw Stone/Bone/source-cargo migration are verified; the aggregate Smithy bench with two selected recipes, finite equipment authority, and 93 generated recipe IDs remain in progress |
| P13 | Client UI for P12: stockpile designation, officer assignment | in progress — designation/assignment, signed manual orders, exact building/farm/gather/road/governance controls, authoritative election timing, durable per-cat typed labor preferences, all nine physical processors' editable add/remove/reorder/repeat/pause queues and truthful inspectors (including the exact missing study for a locked recipe), Steward pile provenance/route visibility, the purchasable 500-study ledger, crop/timber HUD state, visible farm stages, and distinct open station compositions shipped; extend the generic queue UI as broader physical recipes land |
...
| P19 | Item/material economy: crafting chains, traders, coin | in progress — the canonical source/station/taxonomy contract now reconciles P12 and P16 while preserving stable IDs and open-top buildings; raw Stone and Bone, physical quarry Stone/rubble/Ore, hunt Hide/Bone, and forage Fibre cargo, finite fresh Fish, physical scaffold inputs, eleven station recipe descriptors/default queues, nine physical processors through Clothier, useful tools, trade goods, a shrine-visiting physical trader with finite persisted stock/purse/wagon, coin, and stable weighted item units with wear/break/repair are live. The aggregate Smithy bench with two selected recipes, finite functional-equipment authority, recipe/material breadth, and broader local inventories remain |
...
- **P12.4a/b workshop chains + Accountant direction** — workshop crafting covers
  ...
  harvest baskets, local handoff, and final finite-storage delivery. Physical Fibre forage now
  follows the same delivery boundary; Smithy alone remains a colony-global aggregate bench.
  ...
  Ore, hunt Hide, and foraged Fibre are carried cargo. Stone Prep, Woodworking, Tannery, and
  Clothier are physical; next convert Smithy to a one-worker physical queue; then
...
- **P12.6 logistics** — general/limited stockpile designation, signed manual shrine orders, and
  ...
  seeded finite general storehouse and complete physical Mill/Sawmill/Wood Cutter/Stone Prep/Woodworking/Workshop/Smelter/Tannery/Clothier routes are
  verified. An appointed Steward creates nine provenance-distinct exact-resource piles for one of
  ...
  blocked recovery persists without overfill. Smithy still needs the same contract.
...
- A full-page 500-study ledger with dependencies, filter/search/pan/zoom. Every study supports
  ...
  drives placement denial text. Eleven maintained station recipes have exact descriptor/catalog
  ownership. Eight recipes are research-gated and three are founding baselines; nine execute
  physically, while the aggregate Smithy bench retains two selected compatibility recipes pending
  conversion. Ninety-three generated recipes and 64 resources remain open.
...
- **Physical-depth boundary:** scaffold costs are pinned at plan time and finite Lumber or Planks
  ...
  progress. Authored stone-road actions still paint and charge the complete path instantly; physical road
...
- Visible hauling now uses the same semantic authority: every one of the 23 `CarryingKind` values
...
- Remaining breadth includes the physical Smithy queue; Bone item variants plus
  finite gem/clay/metal raw stacks and variants; finished functional
  tool/weapon/armor chains, broader physical local inventories, and reachable exact client
  controls for every chain.
- Physical-economy follow-ups remain explicit: Smithy still needs the station-local
  carry/work/delivery contract; climate resource hints must become real
...
| --- | --- | --- |
| P19.C1 — physical source taxonomy | done | Defaulted raw Stone preserves legacy Materials as distinct Supplies across saves, wire, storage, trade, HUD, and private Accountant reports. Quarry returns three Stone loads plus renewable rubble/Supplies; only persisted Mountains add an Ore load. Hunts return three Food loads followed by distinct Hide and Bone cargo; Bone is independently defaulted through save, wire, storage, stockpiles, trade, HUD, and Accountant projections. Aggregate credit waits for finite delivery, partial logging depletes on first extraction, and death/cancel/restart/full-storage cases conserve cargo. Five-seed unattended faucet/population campaigns, passive Bone hunts, a signed fresh-zero-Stone quarry, and the signed player road→farm→Mill replay verify both no-input and player-guided play. The 20-Supplies offering bar, completed-Field essential floor, and topology-signature construction-route cache are regression-covered. The final 1,169 sim + 43 protocol + 82 server + 134 client tests, strict four-crate Clippy, and accepted exact 1024×768 client-owned `/tmp/raw-stone-bone-final.png` showing counted Stone `~12/100` and Bone `~3/100` verify the slice; Bone item variants remain open breadth. |
| P19.C2 — remaining physical stations | in progress | C2.0 gives all six benches stable data-owned recipe descriptors, canonical resource sets, deterministic default queues, exact selected-recipe catalog/block metadata, rules-v0 grandfathering, and generic signed/persisted queue state. Across all maintained stations there are eleven runtime recipes: eight research-gated and three founding baselines. C2.1 completes Wood Cutter's finite five-Logs→one-Plank route. C2.2 completes Stone Prep: five Stone travel to local input, one Process worker remains assigned across fetch/work/output and advances the selected 600-second `stone_to_blocks` batch, and one local Block travels to finite storage before aggregate credit. C2.3 completes Woodworking: one Craft worker sequentially fetches two Planks and two Blocks, consumes both local inputs atomically in one selected 600-second batch, and carries one whole scalar Tool from local output to finite storage before aggregate credit. C2.4 completes Tannery: five physically hunted Hide travel through inbound and local input, one Textile worker advances the selected 600-second `hide_to_leather` batch, and one Leather travels through local output and outbound cargo before finite-store credit. C2.5 completes the ninth physical route: forage leaves Fibre in paws, one Textile worker carries five Fibre into Clothier, advances the selected 600-second `fibre_to_cloth` batch, and carries one Cloth to finite storage; the local store remains fixed at 10. The complete route is verified at 1-second, 5-second, and 60-second cadence; five signed guided forages verify the source, an authentic HMAC campaign crosses SQLite restart, and established passive runs produce at both 60-second and 5-minute cadence. Earlier C2 slices cover independent construction reserves, comfort release, whole-unit capacity, queue controls, full storage, death/removal, exact restart, no-seed queue preservation, legacy-timer freezing, and inspector truth. The accepted Tannery framebuffer remains `/tmp/physical-tannery-c2.png` (SHA-256 `260915ad91a1c8a69302a1508590cb1abe685831b43a686bd7cd44581a9502ff`). The accepted exact 1024×768 Clothier framebuffer `/tmp/physical-clothier-c2.png` (SHA-256 `2c00b7edaacb24037c81acb3d8bc262e9a4f165163c1ccf64326288f4e649ccd`) shows distinct Fibre/Cloth art and every physical stage/control without clipping. Final gates pass 1,226 simulation, 44 protocol, 91 server, and 136 client tests plus strict four-crate Clippy, formatting, and diff checks. Finite Tool identity remains C3. Smithy is next: it is the sole aggregate bench and retains two selected recipe IDs. |
