...
| Canonical production contract | P19 owns the stable taxonomy and physical source/station boundary. Fibre now leaves forage in paws; Clothier carries five Fibre through one selected 600-second Textile batch and one Cloth outbound before aggregate credit. Only Smithy remains an aggregate compatibility bench; finished equipment still has scalar compatibility fields plus incomplete finite-item chains. | in progress | Rules-v0–v5, capacity/conservation, persistence, signed/passive campaigns, and processor framebuffers cover the converted contract; Clothier specifically has 1s/5s/60s route, five signed-forage, authentic HMAC restart, passive 60s/5m evidence, and an accepted exact 1024×768 selected-station framebuffer. Convert Smithy; add sourced breadth; migrate equipment authority without double-counting. |
| Physical workshop logistics | Nine processors are physical, including Fibre→Cloth and Hide→Leather textile routes. Aggregate credit waits for final delivery; queue/transit/local state persists; capacity, death, removal, and replacement workers conserve cargo. Smithy alone still uses aggregate inputs/outputs. | in progress | Nine-station provenance, capacity, recovery, restart, live-cadence, signed controls, and determinism are verified; extend the contract to Smithy |
...
| Workshop inspector | Mill, Sawmill, Workshop, Smelter, Tannery, and Clothier snapshots and the Bevy inspector expose the real worker, travel direction, editable queue, local input/output, inbound/outbound cargo, progress, and blocked reason. Steward-managed pile inspectors expose active/dormant ownership, resolved station type, route phase, worker, and blocked recovery; unselected managed zones keep their typed overlays/props without persistent text plaques. | verified | Six station persistence/queue paths and Steward protocol/client coverage pass; accepted own-framebuffers show Mill local/outbound Flour, Workshop Refined in route, Smelter Metal in route, Tannery Hide/Leather in route, Clothier Fibre/Cloth in route, and a selected Steward-managed Sawmill Logs pile with readable provenance and no overlap |
...
| Production breadth | The maintained production chains include physical Fibre→Cloth and Hide→Leather. Smithy remains the sole aggregate/parallel-cycle bench; broader recipes and variants remain. | in progress | Convert Smithy, then build/staff/produce sourced recipe breadth from player actions |
...
| `docs/ARCHITECTURE.md` | reconciled | Keep phase/action/test inventories and physical seven-processor/Steward/Accountant behavior synchronized with code |
...
| `p12-idle-cat-forest.md` | partial | Seven specialist officer roles plus the bounded Leader safety floor, role-building gates, signed manual work, shrine reachability, useful tools/costs, physical and wire-confidential Accountant reporting, nine physical processors through Clothier, Steward-managed local reserves, all 19 maintained skill gain/effect/UI paths, and physical Forester replanting/regrowth are verified; convert the aggregate Smithy bench and extend local inventories under the P19-canonical source contract |
...
| `p19-items-materials-trade.md` | canonical taxonomy, implementation partial | Its production table now resolves P12/P16 terminology while preserving stable IDs and every existing open-top station. Raw Stone and Bone are distinct defaulted physical cargo across save/wire/storage/trade/HUD; quarry rubble, mountain Ore, hunt Hide/Bone, foraged Fibre, finite fresh Fish, nine physical processors through Clothier, scaffold delivery, weighted item-unit wear/break/material-repair, and the physical finite shrine trader with exact controls are live. Remaining work is the aggregate Smithy bench with two selected recipes, Bone and broader material/item recipes, finite functional-equipment authority, deeper inventories, and transport. |
