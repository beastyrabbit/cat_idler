...
| Client visibility | Crop/timber resources, farm stages, carried logs, fresh Fish plus habitat stock/capacity, Mill/Sawmill, and all 19 typed skills round-trip and render in the HUD/world or inspector. Ore/metal and the remaining material/item breadth still need a complete mature-colony visibility audit. | in progress | Skill protocol/inspector tests, Fish HUD/shore-tooltip tests, and the fishing designation control framebuffer are verified; inspect every remaining resource/item in a mature-colony framebuffer campaign |
| Transport and fishing | The first physical shoreline route is verified: signed designation, Farmer-only automation, worker travel/work/return, work-only timing, finite fresh-Fish cargo transfers, typed Fishing/Haul skill, cancellation/death conservation, hard reachability, exact restart state, actual general-storehouse-footprint delivery, and guided/unattended determinism. A persisted habitat keyed by canonical water tile holds at most 24 fish and regenerates by 0.5 per game-hour. Only successful on-site catches deplete it; removing/repainting the shore never resets it. Cargo at the village anchor is not credited, full storage leaves it physically waiting, and no catch becomes generic Food. Fine-biome movement factors remain unused; rail has no tracks/trains and shipping has no vessels. | in progress | Focused ecology/depletion/replenishment, storage, routing, removal, death, protocol/client, and SQLite restart regressions plus three-seed live-cadence guided/unattended exact twins verify the route. The campaign uses a natural revealed shore when available and otherwise installs a test-only River neighbour, so it is fixture-assisted rather than worldgen evidence. Exact biome travel plus built rail/ship routes and vehicles remain. |
