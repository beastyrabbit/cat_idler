...
| Production breadth | The maintained production chains include physical Fibre→Cloth, Hide→Leather, selected Metal→Tool/Weapon/Armor, and ten exact Bone/Gem/Clay/Sand goods. Broader sourced recipes and variants remain. | in progress | Extend the same finite source/station/item contract to remaining selected breadth |
...
| Research effect truth | The 487-node dependency graph, supported purchases, persistence, daily Leader choice, and 169 global scalar modifiers are live. Physical yield and durability modifiers have real consumers. Twelve capacity targets cover general Food/Water/Smithy storage plus nine processor-local input/output/transit stores; thirteen containerless studies are omitted with one-time refunds. Twenty-three runtime recipes are physical: twenty research-gated and three founding baselines, including split Mill operations, Smithy Tool/Weapon/Armor, and ten Bone/Gem/Clay/Sand variants. Twelve Crews studies add independently owned worker/provenance/queue/pause/progress/skill/tool stations while sharing finite building stores in stable order. Thirteen unsafe/passive Crews families and 81 recipe plus 64 resource promises remain visible, non-purchasable `FUTURE` content and do not block later studies. | in progress | Physical payloads, signed/passive routes, death/reassignment/restart, FUTURE UI/denial, and the recipe/resource matrix are verified. Add remaining sourced breadth; activate another Crews node only with a real concurrent labor state machine. |
...
| Client visibility | All 28 maintained scalar resources have unique semantic art, labels, paths, tints, and live values without aliases. Every one of the 28 physical `CarryingKind` values reuses its exact tracked HUD identity above the moving cat; there is no colored-square fallback. Stockpile totals, inspectors, Goods groups, farm stages, Fish habitat state, stations, and skills are visible. Stone uses a tracked block/ingot glyph, Bone a tracked fish-skeleton glyph, and Gem/Clay/Sand use distinct public-pack piles. | verified | The exhaustive 28-entry protocol/HUD bijection and cargo map resolve tracked semantic PNGs, reject byte aliases and missing/non-PNG files, and keep all raw and processed identities distinct. Inspected own-framebuffers verify the supported client bounds without overlap, fallback squares, or clipping. |
...
| Item and recipe breadth | Live crafting now includes selected Bone tools/decor/toys, Gem jewelry, Clay pottery/bricks, and Sand glassy goods. Each is a finite stable identity with authoritative material, quality, condition, location, persistence, and trade visibility. The remaining 81 generated recipes and 64 generated resources remain future breadth. Functional Tool/Weapon/Armor chains retain one finite-object authority; stable scalar fields are derived compatibility projections. | in progress | Add only design-selected combinations with the same complete physical contract |
...
| Research tree scale and UX | The truthful about-500 catalog provides exactly 487 nodes: 165 building, 167 recipe/resource, and 155 upgrade studies with stable IDs, typed payloads, AND prerequisites, and layout coordinates. Both product categories exceed the one-third design floor. Supported studies are purchasable and persistent; unsupported generated promises remain visible as disabled `FUTURE` cards. Thirteen former containerless capacity nodes are absent with migration refunds. The living Leader chooses at most one affordable supported study per rolling real-life day. Twenty-three maintained runtime recipes are authoritative: twenty research-gated and three founding baselines. | verified | Quantitative category guards, supported dependency-order purchase, atomic future denial, exact 81 recipe/64 resource future count, migrations, signed server/SQLite behavior, deterministic Leader selection, 23 descriptors, research UI/framebuffer, and strict gates |
...
| `p19-items-materials-trade.md` | canonical taxonomy, implementation partial | Stable source/station names, open-top stations, all ten processors, 23 selected physical recipes, ten exact Bone/Gem/Clay/Sand goods, finite functional equipment, scaffold delivery, wear/repair, and physical trade are live. Remaining work is the 81 recipe/64 resource future breadth in `RECIPE_RESOURCE_MATRIX.md` and deeper inventories. |
