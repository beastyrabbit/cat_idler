...
| Research effect truth | The 487-node dependency graph, supported purchases, persistence, daily Leader choice, and 169 global scalar modifiers are live. Physical yield and durability modifiers have real consumers. Twelve capacity targets cover general Food/Water/Smithy storage plus nine processor-local input/output/transit stores, while thirteen containerless studies are omitted with one-time legacy refunds. Thirteen runtime recipe IDs have catalog-derived descriptors: ten research-gated and three founding baselines. Grain→Flour and Flour→Food are separate Mill selections; Toolmaking, Weaponsmithing, and Armorsmithing independently gate Smithy Tool, Weapon, and Armor work. Unsupported registry promises are visible `FUTURE` content and cannot spend points or consume the Leader's daily choice. The other 91 generated recipe IDs, all 64 generated resource IDs, and all 25 worker-slot modifiers remain incomplete. | in progress | Physical yield, durability repair, twelve capacity targets, exact trade isolation, passive processor cadence, v7 queue/SQLite migration, signed farm→Mill and Ore→metal-Tool campaigns, passive metal-Tool twins, thirteen descriptor bindings/ten research gates/three founding baselines, FUTURE UI/denial, and `RECIPE_RESOURCE_MATRIX.md` are verified. Add sourced recipe/resource breadth and real multi-worker consumers. |
