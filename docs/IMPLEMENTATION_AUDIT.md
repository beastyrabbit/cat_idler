...
| Transport and fishing | Fishing is a maintained physical shoreline chain: signed player designation, Farmer automation, worker travel/work/return, three conserved food cargo trips, finite pile storage, Fishing/Haul skill, cancellation/death salvage, and SQLite restart continuity. Fine-biome movement factors remain unused; rail has no tracks/trains and shipping has no vessels. | in progress | Fishing is verified by typed boundary tests, a signed server campaign, persistence twin, death/conservation tests, 3-seed live-cadence guided/unattended determinism campaign, four-crate gates, and an inspected own-framebuffer control capture. Exact biome travel plus built rail/ship routes and vehicles remain. |
...
| `p16-village-blueprint.md` | partial | The 15-adult/three-five-bed-Den lifecycle, founding clearing, exterior water, exact roads/gate, selectable/removable gather controls, physical fishing, and exterior agricultural territory are verified; broader physical production chains remain |
...
| `p19-items-materials-trade.md` | partial | Complete per-material items/recipes, broader physical inventories, transport, and expose every remaining chain to play; fishing is physical and verified |
...
| Historical roadmap stretch items such as traveler interception and elevation-aware zones | Not current commitments. Real rail/ship transport was later promoted by P17/P19 and remains open above; physical fishing is now implemented. |
