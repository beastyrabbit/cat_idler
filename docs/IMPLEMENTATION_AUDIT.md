...
| Research effect truth | The 487-node dependency graph, purchases, persistence, daily Leader choice, and 169 global scalar modifiers are live. The legacy `gatherYieldMult` scales physical Fibre forage, while `materialYieldMult` scales physical logging and quarry loads before deterministic trip splitting; neither changes job completion time or bypasses hauling/capacity. The durability multiplier is consumed by material-backed item repair. Food Storage, Water Bowl, and Smithy capacity modifiers have target-correct physical domains shared by clamp, routing, snapshot, trade, and persistence; no building bonus leaks globally. Workshop, Mill, Sawmill, Wood Cutter, Stone Prep, Woodworking, Smelter, Tannery, and Clothier capacity studies now expand only their owning persisted input/output/transit stores from 10 to 12 units per accepted resource, controlling delivery and production headroom and visible in the inspector. Thirteen generated capacity studies with no routed container are omitted rather than sold as inert/global bonuses; exact historical point costs are refunded once on old saves. Eleven maintained runtime recipe IDs have exact catalog-derived descriptor availability: eight are research-gated and three founding baselines are immediately available. Four preparation payloads enforce the physical Mill/Sawmill/Workshop/Smelter queues; Textiles enforces physical Fibre→Cloth and Hide→Leather, while Weaponsmithing and Armorsmithing independently enforce the physical Smithy's selected Weapon and Armor recipes. Fresh rules-v1 production cannot advance those eight research-gated paths before entitlement; legacy rules-v0 saves are grandfathered. The Wood Cutter, Stone Prep, and Woodworking studies gate later recipes rather than blocking their founding chains. Sawmill→Gather Logs is the sole validated job entitlement and drives both signed work and Forester automation; false founding/non-runtime job claims are gone. Research Hut plus the three P16 benches are explicitly founding-placeable; `milling` is the sole Mill placement unlock and generated Mill Foundations is durability only, all through one catalog-derived resolver. The bench markers coexist with their first durability modifiers and never require purchase. The other 93 generated recipe IDs, all 64 generated resource IDs, and all 25 worker-slot modifiers remain incomplete; 42 of 143 remaining building modifiers have a target-correct observable consumer. Owning every node therefore does not yet produce every promised gameplay effect. | in progress | Upgraded-vs-control forage/logging/quarry yields, physical multi-trip campaigns, exact completion time, durability-aware repair, twelve exhaustive physical capacity targets, signed capacity purchase/persistence, exact trade isolation, passive processor cadence, catalog-wide resolver equivalence, deterministic twins, eleven descriptor bindings/eight research gates/three founding baselines, unique job/building sources, exact Leader authority/boundary/restart behavior, and the exhaustive payload/consumer map are verified. Add only sourced recipe/resource breadth and real multi-worker consumers with signed, persistence, daily-choice, and player-visible campaigns. |
...
| Client visibility | All 25 maintained scalar resources have unique semantic art, labels, paths, tints, and live values without aliases. Every one of the 25 physical `CarryingKind` values reuses its exact tracked HUD identity above the moving cat; there is no colored-square fallback. Stockpile totals, inspectors, Goods groups, farm stages, Fish habitat state, stations, and skills are visible. Stone uses a tracked block/ingot glyph, Bone a tracked fish-skeleton glyph, Leather its tracked hide-derived goods glyph, and raw Fibre uses the public-pack haystack silhouette rather than aliasing the Flour pouch. | verified | The exhaustive 25-entry protocol/HUD bijection and exhaustive 25-entry cargo map resolve tracked PNGs under `public/images/game/icons/` plus Fibre's exact `public/images/game/props/haystack.png`, reject aliases and missing/non-PNG files, and keep Fibre, Flour, Cloth, Lumber, and Planks distinct. Inspected exact 1024×768 client-owned framebuffers verify all HUD rows and representative cargo loads without overlap, fallback squares, or clipping. |
...
| Item and recipe breadth | The item taxonomy is broad, but live crafting covers a small wood/stone/cloth/leather subset. Raw Bone is sourced physically and persists/trades independently, while bone/gem/clay/sand variants and most generated recipes/resources remain incomplete. Functional Tool/Weapon/Armor chains now have one finite-object authority; stable scalar fields are derived compatibility projections. | open | Source and craft every maintained material/category combination while preserving the verified finite equipment authority across quality, inventory, use, repair, trader, protocol, persistence, and UI |
...
| Research tree scale and UX | The truthful about-500 catalog provides exactly 487 nodes: 154 building, 167 recipe/resource, and 166 upgrade studies across named families, with stable IDs, typed payloads, AND prerequisites, and layout coordinates. Every listed study is purchasable with research points and persists through SQLite; thirteen former containerless capacity nodes are deliberately absent. The living Leader deterministically chooses at most one affordable study per rolling real-life day without taking research labor or rituals from the Loremaster. The full-page client ledger renders and purchases the graph, labels the deterministic hint as “Leader priority,” gives maintained recipes human names, and keeps legacy blessing commissions limited to original nodes. Eleven maintained runtime recipe descriptors, the sole Sawmill logging entitlement, and founding/researched building placement are authoritative: eight recipes are research-gated, three are founding baselines, and all eleven execute physically. The remaining future-content registries are not yet gameplay. | verified | Exhaustive 487/487 dependency-order purchase, signed server action, retired-node refund migration, legacy-column SQLite boundary restart, deterministic Leader selection/replacement/reset, research UI and framebuffer, eleven descriptor bindings/eight research gates/three founding baselines, unique runtime job/building sources, and strict sim/server/client gates |
...
| `p19-items-materials-trade.md` | canonical taxonomy, implementation partial | Its production table now resolves P12/P16 terminology while preserving stable IDs and every existing open-top station. Raw Stone and Bone are distinct defaulted physical cargo across save/wire/storage/trade/HUD; quarry rubble, mountain Ore, hunt Hide/Bone, foraged Fibre, finite fresh Fish, all ten physical processors, scaffold delivery, weighted item-unit wear/break/material-repair, and the physical finite shrine trader with exact controls are live. Remaining work is Bone and broader material/item recipes, finite functional-equipment authority, deeper inventories, and transport. |
...
| Blessings, buildings, and the original ~18-node research tree | Carried forward and expanded: active shrine faucets, useful tools, escalating costs, all 487 truthful research-point purchases, deterministic Leader-owned full-catalog daily selection, persistence, and modeled effects are verified. The durability payload affects real item repair, four preparation studies gate physical recipes, and building-placement claims are authoritative. Research labor/building automation and rituals remain Loremaster-owned. Extra worker-slot, resource, recipe, and job records remain open where no corresponding runtime system exists. |
