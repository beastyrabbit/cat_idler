...
| Client visibility | All 31 maintained scalar resources have unique semantic art, labels, paths, tints, and live values without aliases. Every physical `CarryingKind` reuses its exact tracked HUD identity above the moving cat; there is no colored-square fallback. Stockpile totals, inspectors, Goods groups, farm stages, Fish habitat state, stations, and skills are visible. Preserves, Medicine, and Brew use distinct public-pack crate, healing, and barrel art. | verified | The exhaustive 31-entry protocol/HUD bijection and cargo map resolve tracked semantic PNGs, reject byte aliases and missing/non-PNG files, and keep all raw and processed identities distinct. The final decoded exact-1024×768 native pair shows all 31 HUD rows above a non-overlapping complete toolbar and a 487/487 research ledger with no `FUTURE` state. |
...
are maintained below; the exact 31-resource/zero-`FUTURE` 1024×768 visual gate is closed.
...
| `p18-visual-polish.md` | verified | Open stations, Accounting Tent, label-free map, timber walls, exterior agriculture, Adventure skin, and unique semantic glyphs for all 31 resource readouts are implemented. The final exact 1024×768 native pair was decoded and inspected with the complete 31-resource HUD, clear toolbar, and 487/487 zero-`FUTURE` research state. |
