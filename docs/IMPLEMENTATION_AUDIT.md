...
| Research effect truth | The 487-node dependency graph, supported purchases, persistence, daily Leader choice, and 169 global scalar modifiers are live. All generated recipe and resource payloads have exact physical descriptors or yield/input, durability/spoilage, cycle, or finite-capacity consumers; 104 runtime recipes are physical. Twelve Crews studies add independently owned worker state. Thirteen unsafe/passive Crews families remain visible, non-purchasable `FUTURE` content. | in progress | Recipe/resource breadth, signed/passive routes, exact identity, restart, and the matrix are verified. Activate another Crews node only with a real concurrent labor state machine. |
...
| Research tree scale and UX | The truthful about-500 catalog provides exactly 487 nodes: 165 building, 167 recipe/resource, and 155 upgrade studies with stable IDs, typed payloads, AND prerequisites, and layout coordinates. All 474 studies with real consumers are purchasable and persistent; the thirteen unbacked Crews studies remain disabled `FUTURE` cards. Thirteen former containerless capacity nodes are absent with migration refunds. The living Leader chooses at most one affordable supported study per rolling real-life day. All 104 maintained runtime recipes are authoritative. | verified | Quantitative category guards, supported dependency-order purchase, atomic future-Crews denial, migrations, signed server/SQLite behavior, deterministic Leader selection, descriptors, research UI, and strict gates |
