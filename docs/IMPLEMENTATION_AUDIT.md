...
| Accountant wire confidentiality | The maintained UI reads the stale aggregate and per-pile reports, but owner snapshots simultaneously expose authoritative `ColonySnapshot.resources`, exact `StockpileSnapshot.contents`, and an `accurate` oracle. A player can therefore bypass physical counting by inspecting the WebSocket payload. | open | Serialize a stale/vacant/blocked village for its authenticated owner and prove that neither exact totals, exact pile contents, nor an equality oracle cross the player-facing wire; retain signed actions, restart, and server-side authority |
...
| `p12-idle-cat-forest.md` | partial | Seven specialist officer roles plus the bounded Leader safety floor, role-building gates, signed manual work, shrine reachability, useful tools/costs, physical Accountant/farm/Mill/Sawmill/Workshop/Smelter work, Steward-managed local reserves, and all 19 maintained skill gain/effect/UI paths are verified; close the Accountant player-wire leak, implement Forester replanting, and finish the P19-canonical source cargo, remaining physical benches, and local inventories |
