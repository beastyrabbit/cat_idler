...
| Research tree scale and UX | The complete 500-node catalog provides 167 building, 167 recipe/resource, and 166 upgrade nodes across named families, with stable IDs, typed payloads, AND prerequisites, and layout coordinates. Every study is purchasable with research points and persists through SQLite. The full-page client ledger renders and purchases the graph; legacy blessing commissions remain limited to original nodes. Four physical recipe entitlements and the founding/researched building-placement declarations are authoritative; the remaining future-content registries are not yet gameplay. Runtime daily choice is deterministic but still assigned to the Loremaster instead of the design's Leader, and “Studying next” is only a priority hint. | in progress | Exhaustive 500/500 dependency-order purchase, signed server action, SQLite round trip, deterministic daily selection, research UI, four recipe bindings, unique building placement sources, and strict sim/server/client gates are verified; correct daily authority/copy and bind only real future content |
...
| `p12-idle-cat-forest.md` | partial | Seven specialist officer roles plus the bounded Leader safety floor, role-building gates, signed manual work, shrine reachability, useful tools/costs, physical Accountant/farm/Mill/Sawmill/Workshop/Smelter work, Steward-managed local reserves, and all 19 maintained skill gain/effect/UI paths are verified; finish broader physical workshop logistics and local inventories |
...
| Blessings, buildings, and the original ~18-node research tree | Carried forward and expanded: active shrine faucets, useful tools, escalating costs, all 500 research-point purchases, deterministic full-catalog daily selection, persistence, and modeled effects are verified. The durability payload affects real item repair, four preparation studies gate physical recipes, and building-placement claims are authoritative. Extra worker-slot, resource, recipe, and job records remain open where no corresponding runtime system exists; daily selection also still belongs to the Loremaster rather than the design's Leader. |
