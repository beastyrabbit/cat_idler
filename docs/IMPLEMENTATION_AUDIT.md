...
| Multi-village model | One canonical global village remains viewable by anonymous sockets and controllable by every authenticated player. Each stable signed identity can found exactly one deterministic distant personal village; ownership, selected routing, and foreign denial survive reconnect/restart. Explicit scout-delivery provenance creates mutual summary-only contact. Accepted offers debit exact source piles or finite equipment identities into durable escrow and create a visible explicit caravan actor that crosses world coordinates between the known shrines and returns before credit; either controlling village can cancel without loss. | verified | Deterministic site/collision, signed two-player founding/discovery/trade, scalar and collision-qualified exact-item manifests, condition preservation, cancellation/full-storage/cadence/no-fog-leak, transactional SQLite restart, projection, controls, and framebuffer |
| Shared-world depth | A durable `Communal` hub and exact personal villages share one canonical mutable tile/Fish authority while fog/contact remain private. Barter travels physically and exact Tool/Weapon/Armor ownership transfers without identity collision. | in progress | Shared overlap, legacy migration, physical caravans and persisted route waypoints are verified; install obstacle-aware shared route output and add encounter/trade depth |
