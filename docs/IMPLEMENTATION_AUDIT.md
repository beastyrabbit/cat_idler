...
| Client visibility | All 25 maintained scalar resources have distinct labels, paths, tints, and live values without aliases. Stockpile totals, inspectors, Goods groups, farm stages, cargo, Fish habitat state, stations, and skills are visible. The semantic icon contract is incomplete: the prior 11 terrain/farm/prop/furniture substitutions remain, Stone uses a world stone pile, and Bone uses a generic goods glyph, for 13 resource rows without resource-specific semantic art. | in progress | Existing identity/value/tooltip/cargo tests and decoded framebuffers verify layout and values; the accepted exact 1024×768 frame shows counted Stone and Bone distinctly. Add one truthful semantic icon per resource, assert every mapping stays under the tracked icon set, and own-framebuffer-check all 25 at supported bounds. |
