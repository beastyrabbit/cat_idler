...
| Canonical production contract | P19 is the resource/taxonomy authority: Logs are raw timber, Planks fine boards, Lumber structural timber, Stone raw rock, Blocks dressed stone, Bone is a distinct hunt byproduct, and stable `materials`/`refined` IDs remain the generic Supplies/Crafted Supplies chain. Raw Stone and Bone are defaulted independently across save, wire, finite storage, trade, cargo, HUD, and private Accountant reports. Quarry returns three Stone loads plus rubble/Supplies and mountain-only Ore; hunts return three Food loads followed by Hide and Bone, with aggregate credit only after delivery. P12 owns officer/manual logistics and P16 owns three placement-available founding benches whose future research gates recipes rather than extra copies. All eleven maintained station recipes have stable data-owned IDs/resource domains/default queues and catalog-derived availability. Wood Cutter, Stone Prep, Woodworking, and Tannery execute selected recipes through station-local one-worker work; two benches still report `aggregate_timer_compatibility`, and finished equipment still has scalar compatibility fields plus incomplete finite-item chains. | in progress | Physical source manifests, descriptor uniqueness/resource sets, selected-recipe availability, rules-v0/v1/v2/v3/v4 metadata, interruption/death/full-storage conservation, legacy persistence, trade, HUD, signed/passive deterministic campaigns, and exact framebuffer truth are verified. Convert Clothier and Smithy to physical one-worker queue execution; add Bone/item recipe breadth without aliasing; migrate equipment authority without double-counting. |
| Physical workshop logistics | Eight processors are physical. Staffed Wood Cutter, Stone Prep, Woodworking, and Sawmill carry finite-store Logs, Stone, Planks, or Blocks through local input/work/output into Planks, Blocks, scalar Tools, or Lumber; a Mill does the same for Grain and intermediate Flour before Flour/Food leave for finite storage; Workshop, Smelter, and Tannery physically route Materials→Refined, Ore→Metal, and Hide→Leather. Woodworking fetches its two inputs sequentially and consumes them atomically. Aggregate credit waits for final delivery, queues and transit/local inventories persist, a destination that fills en route triggers physical retargeting, and death/removal conserve cargo at its real location. An appointed Steward creates exact-resource local reserves and balances input deficits before output surplus through one conserved route; vacancy leaves dormant contents rather than teleporting them. Clothier and Smithy still use aggregate inputs/outputs. | in progress | All-eight-station provenance/fairness, capacity, no-early-credit, full-destination recovery, cancellation/death/removal, SQLite restart, live-cadence, queue, signed controls, cadence-partition, and determinism coverage are verified; extend the contract to Clothier and Smithy |
...
| Workshop inspector | Mill, Sawmill, Workshop, Smelter, and Tannery snapshots and the Bevy inspector expose the real worker, travel direction, editable queue, local input/output, inbound/outbound cargo, progress, and blocked reason. Steward-managed pile inspectors expose active/dormant ownership, resolved station type, route phase, worker, and blocked recovery; unselected managed zones keep their typed overlays/props without persistent text plaques. | verified | Five station persistence/queue paths and Steward protocol/client coverage pass; accepted own-framebuffers show Mill local/outbound Flour, Workshop Refined in route, Smelter Metal in route, Tannery Hide/Leather in route, and a selected Steward-managed Sawmill Logs pile with readable provenance and no overlap |
...
| Production breadth | Deterministic exterior farm plots, logging, grain→flour→food, logs→planks/lumber, Stone→Blocks, physical Planks+Blocks→scalar Tools, physical Hide→Leather, fibre→cloth, ore→metal, maintained staffing, tool productivity, lumber-first construction, persistence, and type-local escalating scaffold costs are verified. Clothier and Smithy remain aggregate/parallel-cycle benches rather than one-worker physical queues; broader recipes and material/item variants also remain. | in progress | Build/staff/produce every remaining building and recipe from player actions; give each remaining bench one selected physical queue recipe; retain pinned plan-time cost and escalation boundaries |
...
| Client visibility | All 25 maintained scalar resources have unique semantic icons, labels, paths, tints, and live values without aliases. Every one of the 21 physical `CarryingKind` values reuses its exact tracked HUD identity above the moving cat; there is no colored-square fallback or world-prop substitute. Stockpile totals, inspectors, Goods groups, farm stages, Fish habitat state, stations, and skills are visible. Stone uses a tracked block/ingot glyph, Bone a tracked fish-skeleton glyph, and Leather its tracked hide-derived goods glyph. | verified | The exhaustive 25-entry protocol/HUD bijection and exhaustive 21-entry cargo map resolve PNGs only under `public/images/game/icons/`, reject aliases and missing/non-PNG files, and keep Lumber distinct from Planks. Inspected exact 1024×768 client-owned framebuffers verify all HUD rows and representative cargo loads without overlap, fallback squares, or clipping. |
...
| `p12-idle-cat-forest.md` | partial | Seven specialist officer roles plus the bounded Leader safety floor, role-building gates, signed manual work, shrine reachability, useful tools/costs, physical and wire-confidential Accountant reporting, farm/Mill/Sawmill/Wood Cutter/Stone Prep/Woodworking/Workshop/Smelter work, Steward-managed local reserves, all 19 maintained skill gain/effect/UI paths, and physical Forester replanting/regrowth are verified; finish the remaining physical benches and local inventories under the P19-canonical source contract |
...
| `p19-items-materials-trade.md` | canonical taxonomy, implementation partial | Its production table now resolves P12/P16 terminology while preserving stable IDs and every existing open-top station. Raw Stone and Bone are distinct defaulted physical cargo across save/wire/storage/trade/HUD; quarry rubble, mountain Ore, hunt Hide/Bone, finite fresh Fish, physical Wood Cutter, Stone Prep, and Woodworking routes, scaffold delivery, weighted item-unit wear/break/material-repair, and the physical finite shrine trader with exact controls are live. Remaining work is the three station-local queues, Bone and broader material/item recipes, finite functional-equipment authority, deeper inventories, and transport. |
