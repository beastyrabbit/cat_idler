...
| Physical workshop logistics | Both maintained processing stations are physical. A staffed Sawmill carries finite-store Logs to station-local input, works on site, and carries Lumber out; a staffed Mill does the same for Grain and intermediate Flour before Flour/Food leave for finite storage. Aggregate credit waits for final delivery, queues and transit/local inventories persist, and death/cancellation conserve cargo. Other benches and workshops still use aggregate inputs/outputs. | in progress | Mill and Sawmill capacity, partial-headroom, nearest-route, no-early-credit, cancellation/death, restart, live-cadence, queue, and determinism coverage are verified; extend the contract to every remaining workshop chain |
| Workshop inspector | Mill and Sawmill snapshots and the Bevy inspector expose the real worker, travel direction, editable queue, local input/output, inbound/outbound cargo, progress, and blocked reason. | verified | Both station persistence paths and queue controls are covered; the accepted 1920×1080 Mill own-framebuffer shows Grain 4.0 local input, Flour 2.0 local output, Flour 1.5 outbound, repeating `grain_to_flour_and_food`, 50% progress, and a worker hauling output |
...
| Research effect truth | The 500-node dependency graph, purchases, persistence, daily Loremaster choice, and 169 global scalar modifiers are live. The legacy `gatherYieldMult` scales explicit fibre forage, while `materialYieldMult` scales physical logging and quarry loads before deterministic trip splitting; neither changes job completion time or bypasses hauling/capacity. The durability multiplier is consumed by material-backed item repair. An exhaustive consumer audit found that all 100 generated recipe IDs, all 64 generated resource IDs, all ten job IDs, and all 25 worker-slot modifiers are registry-only; only 30 of 156 building modifiers have a target-correct observable consumer. All 25 building-capacity modifiers leak the largest completed-building bonus globally and are omitted by snapshot/trade capacity paths. Research Hut and Mill building-unlock payloads also disagree with their bootstrap/direct placement gates. Owning every node therefore does not yet produce every promised gameplay effect. | in progress | Upgraded-vs-control forage/logging/quarry yields, physical multi-trip campaign, exact completion time, durability-aware repair, deterministic twins, and the exhaustive payload/consumer map are verified. Correct target-specific capacity first; bind maintained physical recipes; then implement job/resource gates and real multi-worker staffing with signed, persistence, daily-choice, and player-visible campaigns. |
...
| `docs/ARCHITECTURE.md` | reconciled | Keep phase/action/test inventories and physical Mill/Sawmill/Accountant behavior synchronized with code |
...
| `p12-idle-cat-forest.md` | partial | Seven specialist officer roles plus the bounded Leader safety floor, role-building gates, signed manual work, shrine reachability, useful tools/costs, physical Accountant/farm/Mill/Sawmill work, and all 19 maintained skill gain/effect/UI paths are verified; finish broader physical workshop logistics and local inventories |
...
| `p16-village-blueprint.md` | partial | The 15-adult/three-five-bed-Den lifecycle, canonical founding store mix, one logical 16px-art tile per authoritative grid cell, founding clearing, exterior water, exact roads/gate, selectable/removable gather controls, physical shoreline fishing, and exterior agricultural territory are verified; broader physical production chains remain |
...
| `p19-items-materials-trade.md` | partial | Finite fresh Fish and the stable weighted item-unit wear/break/material-repair loop are live; complete per-material items/recipes, broader physical inventories, transport, and expose every remaining chain to play |
...
| Click-to-feed/heal/assign/fight and a browser task queue | Superseded by signed typed management actions and the verified seven-role manual-to-officer loop; coordinate placement, governance, variant/removal, durable per-cat labor preferences, and editable physical Mill/Sawmill queue controls are live |
| Blessings, buildings, and the original ~18-node research tree | Carried forward and expanded: active shrine faucets, useful tools, escalating costs, all 500 research-point purchases, deterministic full-catalog daily Loremaster selection, persistence, and modeled effects are verified. The durability payload now affects real item repair; extra worker-slot payloads and planned recipe/resource/job IDs remain truthful registries until those corresponding runtime systems exist. |
