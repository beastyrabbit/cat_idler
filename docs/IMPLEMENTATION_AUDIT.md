...
| Canonical production contract | P19 owns the stable taxonomy and physical source/station boundary. All ten maintained processors and all 104 recipes are physical. Every generated recipe/resource payload has an authoritative consumer. Tool/Weapon/Armor identity, location, condition, equipment, work/combat wear, repair, escrow, and exact sale have one finite authority. | verified | Rules-v0–v7 plus finite-equipment rules-v1, all 104 descriptors, zero unsupported recipe/resource payloads, capacity/conservation, persistence, signed/passive campaigns, exact-ID lifecycle, and processor/equipment framebuffers cover the contract. |
| Physical workshop logistics | All ten processors are physical, including Fibre→Cloth, Hide→Leather, and selected Metal→Weapon/Armor routes. Aggregate credit waits for final delivery; queue/transit/local state persists; capacity, death, removal, and replacement workers conserve cargo. | verified | Ten-station provenance, capacity, recovery, restart, live-cadence, signed controls, and determinism are verified |
...
| Research effect truth | All 487 nodes in the dependency graph are purchasable, persistent, and consumed. All 104 runtime recipes are physical, and every generated recipe/resource payload has an exact descriptor or yield/input, durability/spoilage, cycle, or finite-capacity consumer. All 165 Building studies are live: physical yield/durability/capacity/cycle modifiers drive their exact station, twelve Crews studies add independent worker state, and thirteen stable Crews IDs provide completed-building-gated housing, preservation, water, rest, medicine, kitten/elder care, defense, physical Mouse Farm Food, blessing, harvest, combat, or accounting service rather than fake slots. Thirteen containerless capacity studies remain intentionally absent with one-time refunds. | verified | Exhaustive 487/487 dependency-order purchase and zero-FUTURE guards; all 104 descriptors; signed/passive routes; exact identity; building consumer behavior; death/reassignment/restart/full-storage conservation; and the recipe/resource matrix are verified. |
| Founding stockpile contract | P16's live finite personal-village mix is canonical at founding and extinction recovery: 50 food, 100 water, 16 herbs, 60 general materials, 10 planks, and 10 blocks, doubled for the communal blueprint; every other maintained scalar resource starts at zero. | verified | Exact 31-field aggregate and finite general-storehouse content assertions, exact communal 2× scaling, single-store/no-legacy-shrine and repeated-reconciliation checks, bit-exact physical-pile conservation, and deterministic personal/communal extinction recovery |
| Client visibility | All 31 maintained scalar resources have unique semantic art, labels, paths, tints, and live values without aliases. Every physical `CarryingKind` reuses its exact tracked HUD identity above the moving cat; there is no colored-square fallback. Stockpile totals, inspectors, Goods groups, farm stages, Fish habitat state, stations, and skills are visible. Preserves, Medicine, and Brew use distinct public-pack crate, healing, and barrel art. | verified; combined frame pending | The exhaustive 31-entry protocol/HUD bijection and cargo map resolve tracked semantic PNGs, reject byte aliases and missing/non-PNG files, and keep all raw and processed identities distinct. Prior feature and representative-cargo frames are inspected; capture the complete 31-row HUD and zero-`FUTURE` ledger together at exact 1024×768 before claiming that final lower-bound visual gate. |
...
are maintained below; the combined 31-resource/zero-`FUTURE` 1024×768 capture remains final QA.
...
| --- | --- | --- |
| `docs/GAME_VISION.md` | verified current intent | Seven-role ownership, exact controls, physical work, all ten processor types and 104 recipes, all 487 live research studies, the larger communal blueprint, personal villages, and physical trade are verified |
...
| `p12-idle-cat-forest.md` | verified | Seven specialist roles plus the bounded Leader safety floor, role-building gates, signed manual work, shrine reachability, useful tools/costs, physical and wire-confidential Accountant reporting, all ten processor types, 104 recipes, Steward reserves, all 19 skills, and Forester replanting/regrowth are verified |
...
| `p15-playtest-feedback.md` | verified | Roads, exact placement, governance, labor pressure, knowledge-blind shrine-return scouting, passive/guided campaigns, secure villages, shared physical trade, and complete recipe/resource breadth are verified |
...
| `p18-visual-polish.md` | verified; final evidence capture pending | Open stations, Accounting Tent, label-free map, timber walls, exterior agriculture, Adventure skin, and unique semantic glyphs for all 31 resource readouts are implemented. Capture and inspect an exact 1024×768 frame containing the complete 31-resource HUD and zero-`FUTURE` research state before claiming that combined lower-bound frame. |
| `p19-items-materials-trade.md` | verified canonical taxonomy | Stable source/station names, all ten processor types, all 104 physical recipes, all material families, exact finite goods, functional equipment, scaffold delivery, wear/repair, and physical visiting/village trade are live |
