...
| Manual controls | Farm/gather/road painting, selectable crops and gather resources, exact coordinate-selected construction, staffing, clear/remove designations, election ballots, create-or-sign vote-kicks, raw-resource orders, expansion, military defense/training, ritual/tithe/offering, hauling, and research purchase have usable signed client paths. Exact per-cat labor assignment and editable station production queues still lack complete client tools. | in progress | Exact-placement and five-player governance restart campaigns are verified; add per-cat labor/queue tools and exercise every visible control at all target resolutions |
...
| Skills breadth | Eighteen typed labors cover hunting, building, ritual, fighting/training, quarrying, woodcutting, foraging, water, milling/processing/crafting, textiles, metalwork, farming, hauling, research, and scouting. Only truthful work accrues tick-size-independent or completed-cycle XP; bounded skill effects apply to the corresponding production, movement, research, and combat paths. Skills persist and the cat inspector exposes the typed map with legacy role-XP compatibility. | verified | Exhaustive labor-source/effect, no-work, continuous/cycle, officer/manual, persistence, protocol/UI, physical-Sawmill compatibility, determinism, 921-test simulation, and strict Clippy gates |
...
| Client visibility | Crop/timber resources, farm stages, carried logs, Mill/Sawmill, and all 18 typed skills round-trip and render in the HUD/world or inspector. Ore/metal and the remaining material/item breadth still need a complete mature-colony visibility audit. | in progress | Skill protocol/inspector tests are verified; inspect every remaining resource/item in a mature-colony framebuffer campaign |
...
| --- | --- | --- |
| `docs/GAME_VISION.md` | current intent, partially implemented | Seven-role ownership, exact placement/governance tools, and the complete 500-study runtime are verified; finish physical workshop/farm labor, large-global/shared-space depth, and complete production controls |
...
| `p12-idle-cat-forest.md` | partial | Seven strict officer roles, role-building gates, signed manual work, shrine reachability, useful tools/costs, Accounting Tent, and all 18 maintained skill gain/effect/UI paths are verified; finish broader physical workshop/farm logistics and local inventories |
...
| `p15-playtest-feedback.md` | partial | Roads, exact footprints/depth, coordinate placement, governance controls, shrine-return fog, and secure village foundations are fixed; oracle scout targeting conflicts with the random-walk requirement, while physical shared-world depth remains |
| `p16-village-blueprint.md` | partial | The 15-adult/three-five-bed-Den lifecycle, founding clearing, exterior water, exact roads/gate, selectable/removable gather controls, and exterior agricultural territory are verified; fishing and broader physical production chains remain |
...
| Fog, expeditions, path wear, terrain travel cost, walls, and gates | Carried forward; shrine-return resource/general scouting, dirt/stone roads, and staged closed-perimeter expansion with an atomic one-gate cutover are implemented; integrated wall-state framebuffers remain |
| Click-to-feed/heal/assign/fight and a browser task queue | Superseded by signed typed management actions and the verified seven-role manual-to-officer loop; coordinate placement, election UI, and variant/removal tools are live, while exact per-cat assignment and production queues remain open above |
