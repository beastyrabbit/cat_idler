...
| Canonical production contract | P19 is now the resource/taxonomy authority: Logs are raw timber, Planks fine boards, Lumber structural timber, Stone raw rock, Blocks dressed stone, and stable `materials`/`refined` IDs remain the generic Supplies/Crafted Supplies chain. P12 owns officer/manual logistics and P16 owns three placement-available founding benches whose future research gates recipes rather than extra copies. Wood Cutter, Stone Prep, and Woodworking now have unique data-owned founding-placement markers and signed later copies do not require `basic_tools`. Runtime is still only partly aligned: there is no raw Stone resource, Wood Cutter/Stone Prep still use generic Materials, quarry/hunt byproducts can credit aggregates without hauling, and finished equipment has both scalar compatibility fields and incomplete finite-item chains. | in progress | Add defaulted Stone without reinterpreting existing Materials saves; prove physical source cargo and conservation; convert all six remaining benches to physical one-worker queues; migrate equipment authority without double-counting; verify old-save/protocol compatibility, founding stock, trade, persistence, HUD, and deterministic guided/unattended campaigns |
...
| Forester replanting | Logging physically reaches one observed tree, replaces it with a persisted stump, and carries Logs home. The maintained vision also assigns replanting to the Forester, but stump tiles are permanently excluded from future logging and no regrowth state machine exists. | open | Signed/manual and Forester-owned planting, finite carried inputs if required, obstruction/vacancy/death behavior, persisted growth timing, deterministic regrowth, and long-horizon forest sustainability |
...
| Research effect truth | The 500-node dependency graph, purchases, persistence, daily Leader choice, and 169 global scalar modifiers are live. The legacy `gatherYieldMult` scales explicit fibre forage, while `materialYieldMult` scales physical logging and quarry loads before deterministic trip splitting; neither changes job completion time or bypasses hauling/capacity. The durability multiplier is consumed by material-backed item repair. Food Storage, Water Bowl, and Smithy capacity modifiers now have target-correct physical domains shared by clamp, routing, snapshot, trade, and persistence; no building bonus leaks globally. Four data-owned preparation payloads unlock the exact maintained Mill, Sawmill, Workshop, and Smelter queue recipes. Four legacy payloads gate maintained aggregate production: Textiles owns fibre→cloth and hide→leather, while Weaponsmithing and Armorsmithing independently own Smithy weapon and armor output across both forge arms. Fresh rules-v1 production cannot advance or consume inputs before entitlement; legacy rules-v0 saves are grandfathered. Sawmill→Gather Logs is the sole validated job entitlement and drives both signed work and Forester automation; false founding/non-runtime job claims are gone. Research Hut plus the three P16 benches are explicitly founding-placeable; `milling` is the sole Mill placement unlock and generated Mill Foundations is durability only, all through one catalog-derived resolver. The bench markers coexist with their first durability modifiers and never require purchase. The other 96 generated recipe IDs, all 64 generated resource IDs, all 25 worker-slot modifiers, and the other 22 building-capacity modifiers remain incomplete; only 33 of 156 building modifiers have a target-correct observable consumer. Mill/Sawmill/Workshop/Smelter local stores remain fixed at 10. Owning every node therefore does not yet produce every promised gameplay effect. | in progress | Upgraded-vs-control forage/logging/quarry yields, physical multi-trip campaign, exact completion time, durability-aware repair, target-isolated capacity, exact trade placement, catalog-wide resolver equivalence, deterministic twins, all eight maintained recipe-entitlement bindings, unique job/building sources, exact Leader authority/boundary/restart behavior, and the exhaustive payload/consumer map are verified. Add only sourced recipe/resource breadth; then implement remaining storage/resource domains and real multi-worker staffing with signed, persistence, daily-choice, and player-visible campaigns. |
| Founding stockpile contract | P16's live finite personal-village mix is canonical at founding and extinction recovery: 50 food, 100 water, 16 herbs, 60 general materials, 10 planks, and 10 blocks, doubled for the communal blueprint; every other maintained scalar resource starts at zero. | verified | Exact 23-field aggregate and finite general-storehouse content assertions, exact communal 2× scaling, single-store/no-legacy-shrine and repeated-reconciliation checks, bit-exact physical-pile conservation, and deterministic personal/communal extinction recovery |
| Client visibility | All 23 maintained scalar resources have distinct names, paths, tints, and live values without fibre/cloth, hide/leather/ore, or metal aliases. Stockpile totals, inspectors, Goods groups, farm stages, cargo, Fish habitat state, stations, and skills are visible. The semantic icon contract is incomplete: 11 resource paths reuse terrain, farm, prop, or furniture sprites instead of tracked resource glyphs. | in progress | Existing identity/value/tooltip/cargo tests and decoded 1024×768/1280×800 framebuffers verify layout and values. Add one truthful semantic icon per resource, assert every mapping stays under the tracked icon set, and own-framebuffer-check all 23 at supported bounds. |
...
| Item and recipe breadth | The item taxonomy is broad, but live crafting covers a small wood/stone/cloth/leather subset; bone/gem/clay/metal variants and finished tool/weapon/armor item chains are incomplete. Stable scalar equipment fields remain compatibility state while finite item units own condition, so the eventual authority migration must not double-count goods. | open | Source and craft every maintained material/category combination; prove one finite-object authority across quality, inventory, use, repair, trader, protocol, persistence, and UI campaigns |
| Visiting trader lifecycle | A visiting merchant, minimap marker, bounded visit window, coin, item-weight sale cap, and signed buy/sell controls are live. The physical route is incomplete: the trader walks in a straight obstacle-blind line to the south gate rather than pathing to the P19 shrine/market destination, and resource purchases use an effectively unbounded catalog instead of finite visit stock. | in progress | Add obstacle-aware arrival and departure, finite persisted inventory, deterministic restocking, exact purchase depletion, departure/restart conservation, and a guided trader-arrival/trade/restock campaign with client visibility |
...
| Physical finite visiting trader | Each NPC merchant owns a deterministic reachable exterior and follows ordinary obstacle-aware A* through the retained gate to the physical shrine; closed routes wait and replan, Shipping never grants water traversal, and Rail remains neutral. Trading starts only on shrine contact and ends at an exact persisted deadline. A visit carries a deterministic finite resource manifest, finite purse, 100 kg wagon, and exact stable item units bought from the colony. Purchases deplete exact stock and expose sold-out truth; sales conserve item identity and stop at purse/cargo capacity. Arrival, stay, stock, purse, item cargo, departure target, and visit counter survive SQLite restart, and the wagon is removed only after reaching its persisted exterior. | in progress | Focused route/passability/closure/reopen/contact/cadence/determinism tests; 5-seed × 3-colony × 1,024-visit cargo matrix; exact buy/sell conservation and sold-out tests; mid-phase SQLite restart; signed HMAC sale, purchase, restart, depletion, and denial; protocol/client phase/route/deadline/manifest tests; strict touched-crate Clippy and format; seed-41 live-cadence 60h passive twin observed one arrival, shrine trading window, and physical departure with no reset. Broad touched-crate tests and the required client-owned framebuffer remain before acceptance. |
...
| `p12-idle-cat-forest.md` | partial | Seven specialist officer roles plus the bounded Leader safety floor, role-building gates, signed manual work, shrine reachability, useful tools/costs, physical and wire-confidential Accountant reporting, farm/Mill/Sawmill/Workshop/Smelter work, Steward-managed local reserves, and all 19 maintained skill gain/effect/UI paths are verified; implement Forester replanting and finish the P19-canonical source cargo, remaining physical benches, and local inventories |
...
| `p18-visual-polish.md` | partial | Open stations, an integrated Accounting Tent, the label-free map, staged walls/exterior agriculture, and the Adventure panel/button/progress/cursor skin are verified in native and optimized WASM campaigns; 11 of 23 resource readouts still need truthful semantic glyphs rather than terrain/furniture reuse |
| `p19-items-materials-trade.md` | canonical taxonomy, implementation partial | Its production table now resolves P12/P16 terminology while preserving stable IDs and every existing open-top station. Finite fresh Fish, physical scaffold delivery, and weighted item-unit wear/break/material-repair are live; add raw Stone, physical byproduct cargo, remaining station queues, finite functional equipment authority, broader recipes/inventories, a shrine/market trader route with finite stock, transport, and exact player controls |
