...
| Canonical production contract | P19 owns the stable taxonomy and physical source/station boundary. All ten maintained processors are physical. Smithy carries two Metal through one selected 900-second Metalwork batch and carries one whole Weapon or Armor outbound before aggregate credit. Finished equipment still has scalar compatibility fields plus incomplete finite-item chains. | in progress | Rules-v0–v6, capacity/conservation, persistence, signed/passive campaigns, and processor framebuffers cover the converted contract. Smithy specifically has 1s/5s/60s routes, authentic HMAC restart, guided Ore→Metal→Weapon provenance, and passive 60s/5m evidence. Add sourced breadth and migrate equipment authority without double-counting. |
