...
| Physical workshop logistics | Four processors are physical. A staffed Sawmill carries finite-store Logs through local input/work/output into Lumber; a Mill does the same for Grain and intermediate Flour before Flour/Food leave for finite storage; Workshop and Smelter now physically route Materials→Refined and Ore→Metal. Aggregate credit waits for final delivery, queues and transit/local inventories persist, a destination that fills en route triggers physical retargeting, and death/removal conserve cargo at its real location. Other benches and workshops still use aggregate inputs/outputs. | in progress | All four processors have capacity, partial-headroom, nearest-route, no-early-credit, cancellation/death/removal, restart, queue, signed guidance, cadence-partition, and determinism coverage; extend the contract to every remaining workshop chain |
| Workshop inspector | Mill, Sawmill, Workshop, and Smelter snapshots and the Bevy inspector expose the real worker, travel direction, editable queue, local input/output, inbound/outbound cargo, progress, and blocked reason. | verified | All four persistence paths and queue controls are covered. Accepted 1920×1080 own-framebuffers show selected Mill, Workshop, and Smelter local ledgers, repeating recipes, progress, outbound cargo, and physical worker routes. |
...
| Research effect truth | The 500-node dependency graph, purchases, persistence, daily Loremaster choice, and 169 global scalar modifiers are live. The legacy `gatherYieldMult` scales explicit fibre forage, while `materialYieldMult` scales physical logging and quarry loads before deterministic trip splitting; neither changes job completion time or bypasses hauling/capacity. The durability multiplier is consumed by material-backed item repair. Food Storage, Water Bowl, and Smithy capacity modifiers now have target-correct physical domains shared by clamp, routing, snapshot, trade, and persistence; no building bonus leaks globally. An exhaustive consumer audit found that all 100 generated recipe IDs, all 64 generated resource IDs, all ten job IDs, all 25 worker-slot modifiers, and the other 22 building-capacity modifiers are registry-only; only 33 of 156 building modifiers have a target-correct observable consumer. Mill/Sawmill/Workshop/Smelter local stores remain fixed at 10. Research Hut and Mill building-unlock payloads also disagree with their bootstrap/direct placement gates. Owning every node therefore does not yet produce every promised gameplay effect. | in progress | Upgraded-vs-control forage/logging/quarry yields, physical multi-trip campaign, exact completion time, durability-aware repair, target-isolated capacity, exact trade placement, catalog-wide resolver equivalence, deterministic twins, and the exhaustive payload/consumer map are verified. Bind maintained physical recipes next; then implement the remaining storage domains, job/resource gates, and real multi-worker staffing with signed, persistence, daily-choice, and player-visible campaigns. |
...
| `docs/ARCHITECTURE.md` | reconciled | Keep phase/action/test inventories and physical processor/Accountant behavior synchronized with code |
...
| `p12-idle-cat-forest.md` | partial | Seven specialist officer roles plus the bounded Leader safety floor, role-building gates, signed manual work, shrine reachability, useful tools/costs, physical Accountant/farm/Mill/Sawmill/Workshop/Smelter work, and all 19 maintained skill gain/effect/UI paths are verified; finish broader physical workshop logistics and local inventories |
