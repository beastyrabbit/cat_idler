...
| Research effect truth | The 500-node dependency graph, purchases, persistence, daily Loremaster choice, and modeled effects are live. The legacy `gatherYieldMult` now scales explicit fibre forage, while `materialYieldMult` scales the physical logging and quarry loads before deterministic trip splitting; neither changes job completion time or bypasses hauling/capacity. Planned recipe/resource/job IDs, worker-slot modifiers, and durability modifiers remain registries without consumers, so owning every node does not yet produce every promised gameplay effect. | in progress | Upgraded-vs-control forage/logging/quarry yields, physical multi-trip campaign, exact completion time, deterministic twin, and exhaustive legacy scalar-consumer map are verified; finish recipe/resource/job gates, staffing-slot changes, durability/maintenance effects, signed purchases, and daily-choice boundaries |
