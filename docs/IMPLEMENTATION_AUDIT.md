...
| Research effect truth | The 500-node dependency graph, purchases, persistence, daily Loremaster choice, and modeled effects are live. The legacy `gatherYieldMult` scales explicit fibre forage, while `materialYieldMult` scales physical logging and quarry loads before deterministic trip splitting; neither changes job completion time or bypasses hauling/capacity. The durability multiplier is now consumed by material-backed item repair. Planned recipe/resource/job IDs and worker-slot modifiers remain registries without consumers, so owning every node does not yet produce every promised gameplay effect. | in progress | Upgraded-vs-control forage/logging/quarry yields, physical multi-trip campaign, exact completion time, durability-aware repair, deterministic twins, and the scalar-consumer map are verified; finish recipe/resource/job gates, staffing-slot changes, signed purchases, and daily-choice boundaries |
