...
| Research effect truth | The 500-node dependency graph, purchases, persistence, daily Loremaster choice, and 169 global scalar modifiers are live. The legacy `gatherYieldMult` scales explicit fibre forage, while `materialYieldMult` scales physical logging and quarry loads before deterministic trip splitting; neither changes job completion time or bypasses hauling/capacity. The durability multiplier is consumed by material-backed item repair. Food Storage, Water Bowl, and Smithy capacity modifiers now have target-correct physical domains shared by clamp, routing, snapshot, trade, and persistence; no building bonus leaks globally. Four data-owned preparation payloads unlock the exact maintained Mill, Sawmill, Workshop, and Smelter recipes, with fresh-village enforcement and legacy-save grandfathering. The other 96 generated recipe IDs, all 64 generated resource IDs, all ten job IDs, all 25 worker-slot modifiers, and the other 22 building-capacity modifiers remain registry-only; only 33 of 156 building modifiers have a target-correct observable consumer. Mill/Sawmill/Workshop/Smelter local stores remain fixed at 10. Research Hut and Mill building-unlock payloads also disagree with their bootstrap/direct placement gates. Owning every node therefore does not yet produce every promised gameplay effect. | in progress | Upgraded-vs-control forage/logging/quarry yields, physical multi-trip campaign, exact completion time, durability-aware repair, target-isolated capacity, exact trade placement, catalog-wide resolver equivalence, deterministic twins, the four recipe-entitlement bindings, and the exhaustive payload/consumer map are verified. Add only physically sourced recipe/resource breadth; then implement the remaining storage domains, job/resource gates, and real multi-worker staffing with signed, persistence, daily-choice, and player-visible campaigns. |
