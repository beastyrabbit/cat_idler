...
| Staged wall growth | Expansion constructs a durable outer perimeter while the complete old enclosure and exact south gate remain authoritative. Finished prospective segments block travel; death, officer interruption, and restart preserve progress; completing every segment atomically cuts over to one new south gate and retires the old shared edge. | in progress | Interrupted/resumed construction, closure, pathfinding, one-gate cutover, persistence, and Sawmill compatibility are verified; capture integrated before/during/after framebuffers |
| Agricultural territory | Claimed agricultural tiles are a distinct persisted/snapshotted subset excluded from settlement-wall derivation, so exterior farms remain outside both the active and prospective enclosure during expansion. | in progress | Territory class, wall derivation, persistence, and path behavior are verified; complete the guided before/after framebuffer campaign and extend the distinction to future non-farm rural uses |
...
| Founding fog progression | The founding Leader sends a very fast first wood search and later resource/general scouts even before a Loremaster office exists; knowledge still becomes permanent only on shrine contact. | in progress | Seeds 7/42/20,240,712 pass the checked-in four-hour Leader-requester gate and exact 48h one-second passive campaigns with permanent fog growth while research/ritual work stays absent; seed 7 records nine completed Leader scouts. An optimized browser founding run verified Explore, shrine return, and permanent reveal growth from 289 to 394 tiles; complete the equivalent fresh native founding capture |
...
| Client visibility | Crop/timber resources, farm stages, carried logs, Mill/Sawmill, and all 19 typed skills round-trip and render in the HUD/world or inspector. Ore/metal and the remaining material/item breadth still need a complete mature-colony visibility audit. | in progress | Skill protocol/inspector tests and the fishing designation control framebuffer are verified; inspect every remaining resource/item in a mature-colony framebuffer campaign |
| Transport and fishing | The first physical shoreline route is verified: signed designation, Farmer-only automation, worker travel/work/return, work-only timing, finite pile transfers, typed Fishing/Haul skill, cancellation/cargo recovery, hard reachability, restart state, actual general-storehouse-footprint delivery, and guided/unattended determinism. Cargo at the village anchor is not credited, and full storage leaves it physically waiting. It still produces generic Food from a permanent designation; no finite fish source/material, depletion, or replenishment rule exists yet. Fine-biome movement factors remain unused; rail has no tracks/trains and shipping has no vessels. | in progress | Fourteen focused storehouse/conservation/routing/removal regressions, all ten fishing-filter tests (including the three-seed live-cadence guided/unattended twins), and the exact full `cat-sim` gate (969 passed, one intentionally skipped) verify the physical route. The campaign uses a natural revealed shore when available and otherwise installs a test-only River neighbour, so it is fixture-assisted rather than worldgen evidence. Model finite fish stock/material and replenishment next. Exact biome travel plus built rail/ship routes and vehicles remain. |
...
| --- | --- | --- | --- |
| Fog of war and scouting | A new village reveals its exact 13×13 claim plus a two-tile halo. Only purposeful scouts lift provisional fog; wood/food/water/stone and general missions commit knowledge only after a living scout physically returns to the shrine. The first Leader-dispatched wood mission is a bounded fast round trip even before a Loremaster exists. In-flight notebooks persist across SQLite restarts. | in progress | Search/travel/return, 32-seed explicitly dispatched wood, determinism, death/cancel/restart, signed controls, and own-framebuffers are verified; exact 48-hour personal/communal campaigns prove autonomous dispatch, permanent revealed growth, and research/ritual vacancy isolation. Optimized browser founding is verified; complete the fresh native founding capture |
...
| `p14-spatial-placement.md` | partial | Atomic placement/reservations/scaffold recovery, full occupancy, soft obstacles, exact roads, exterior agricultural claims, and staged atomic wall/gate cutover are verified; integrated staged-state framebuffers remain |
...
| `p18-visual-polish.md` | partial | Open stations, Accounting Tent reachability/composition, the label-free map, and the Adventure panel/button/progress/cursor skin are verified in native and optimized WASM campaigns; the integrated Accounting Tent and staged wall/agricultural native captures remain |
| `p19-items-materials-trade.md` | partial | Complete per-material items/recipes, broader physical inventories, transport, and expose every remaining chain to play; fishing still needs a finite fish source/material rather than permanent generic Food |
...
| Fog, expeditions, path wear, terrain travel cost, walls, and gates | Carried forward; shrine-return resource/general scout movement, dirt/stone roads, and staged closed-perimeter expansion with an atomic one-gate cutover are implemented; fresh baseline Leader dispatch and integrated wall-state framebuffers remain |
...
after this capture; their integrated visual sequence remains.
...
and permanent reveal growth; only the equivalent fresh native founding capture remains.
