...
| Construction-material logistics | P19 requires finite Lumber or Planks plus Blocks to reach a scaffold before on-site work. Current plan-time costs and paid-scaffold recovery are stable, but player commit and autonomous break-ground subtract colony aggregates immediately; the builder then travels without construction cargo. | open | Reserve exact finite source units, route visible loads into scaffold-local input, forbid progress before delivery, and prove cancellation/death/removal/no-route/full-storage/restart conservation without changing pinned cost or escalation semantics |
...
| `p12-idle-cat-forest.md` | partial | Seven specialist officer roles plus the bounded Leader safety floor, role-building gates, signed manual work, shrine reachability, useful tools/costs, physical Accountant/farm/Mill/Sawmill/Workshop/Smelter work, Steward-managed local reserves, and all 19 maintained skill gain/effect/UI paths are verified; finish the P19-canonical source cargo, remaining physical benches, and local inventories |
| `p14-spatial-placement.md` | verified placement slice | Atomic placement/reservations/scaffold recovery, full occupancy, soft obstacles, exact roads, exterior agricultural claims, and staged atomic wall/gate cutover are verified in code and accepted native framebuffers. P19's finite scaffold-material delivery remains open; physical authored-road labor is a P2 consistency enhancement under current P16 wording |
...
| `p19-items-materials-trade.md` | canonical taxonomy, implementation partial | Its production table now resolves P12/P16 terminology while preserving stable IDs and every existing open-top station. Finite fresh Fish and weighted item-unit wear/break/material-repair are live; add raw Stone, physical byproduct cargo, scaffold-material delivery, remaining station queues, finite functional equipment authority, broader recipes/inventories, a shrine/market trader route with finite stock, transport, and exact player controls |
