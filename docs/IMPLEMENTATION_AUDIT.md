...
| Canonical production contract | P19 owns the stable taxonomy and physical source/station boundary. All ten maintained processors are physical. Smithy carries two Metal through one selected 900-second Metalwork batch and the resulting stable Weapon/Armor ID travels local-output→carrier→storage before derived scalar credit. Tool/Weapon/Armor identity, location, condition, equipment, work/combat wear, repair, and exact sale have one finite authority. | in progress | Rules-v0–v6 plus finite-equipment rules-v1, capacity/conservation, persistence, signed/passive campaigns, the complete exact-ID lifecycle, and processor/equipment framebuffers cover the converted contract. Add sourced recipe/material breadth without reopening the equipment authority. |
...
| Production breadth | The maintained production chains include physical Fibre→Cloth, Hide→Leather, selected Metal→Tool/Weapon/Armor, ten exact Bone/Gem/Clay/Sand goods, complete Grain Milling/Baking/Herbalism/Food Preservation/Brewing families, and complete Hunting/Foraging/Waterworks/Animal Husbandry/Field Craft/Expedition Supplies catalog families. The latter create exact finite kits and wares for equipment/trade without claiming hidden farm/scout automation. Broader sourced recipes and variants remain. | in progress | Extend the same finite source/station/item contract to the remaining 30 recipe and 27 resource promises |
...
| Research effect truth | The 487-node dependency graph, supported purchases, persistence, daily Leader choice, and 169 global scalar modifiers are live. Physical yield and durability modifiers have real consumers. Twelve capacity targets cover general Food/Water/Smithy storage plus nine processor-local input/output/transit stores; thirteen containerless studies are omitted with one-time refunds. Seventy-four runtime recipes are physical. Beyond complete food/plant families, all fifty Hunting/Foraging/Waterworks/Animal Husbandry/Field Craft/Expedition Supplies nodes now own finite item routes or exact input/durability/cycle/capacity consumers. Twelve Crews studies add independently owned worker state. Thirteen unsafe/passive Crews families and 30 recipe plus 27 resource promises remain visible, non-purchasable `FUTURE` content. | in progress | Physical payloads, signed/passive routes, death/reassignment/restart, FUTURE UI/denial, and the recipe/resource matrix are verified. Add remaining sourced breadth; activate another Crews node only with a real concurrent labor state machine. |
| Founding stockpile contract | P16's live finite personal-village mix is canonical at founding and extinction recovery: 50 food, 100 water, 16 herbs, 60 general materials, 10 planks, and 10 blocks, doubled for the communal blueprint; every other maintained scalar resource starts at zero. | verified | Exact 28-field aggregate and finite general-storehouse content assertions, exact communal 2× scaling, single-store/no-legacy-shrine and repeated-reconciliation checks, bit-exact physical-pile conservation, and deterministic personal/communal extinction recovery |
| Client visibility | All 31 maintained scalar resources have unique semantic art, labels, paths, tints, and live values without aliases. Every physical `CarryingKind` reuses its exact tracked HUD identity above the moving cat; there is no colored-square fallback. Stockpile totals, inspectors, Goods groups, farm stages, Fish habitat state, stations, and skills are visible. Preserves, Medicine, and Brew use distinct public-pack crate, healing, and barrel art. | verified | The exhaustive 31-entry protocol/HUD bijection and cargo map resolve tracked semantic PNGs, reject byte aliases and missing/non-PNG files, and keep all raw and processed identities distinct. Inspected own-framebuffers verify the supported client bounds without overlap, fallback squares, or clipping. |
...
| Item and recipe breadth | Live crafting now includes selected Bone/Gem/Clay/Sand goods, 23 additional food/plant recipes, and 28 additional exact subsistence/frontier kits and wares. All exact goods retain one finite identity through station, cargo, storage, equipment where functional, and trade. The remaining 30 generated recipes and 27 generated resources remain future breadth. Functional Tool/Weapon/Armor chains retain one finite-object authority; stable scalar fields are derived compatibility projections. | in progress | Add only design-selected combinations with the same complete physical contract |
...
| Research tree scale and UX | The truthful about-500 catalog provides exactly 487 nodes: 165 building, 167 recipe/resource, and 155 upgrade studies with stable IDs, typed payloads, AND prerequisites, and layout coordinates. Both product categories exceed the one-third design floor. Supported studies are purchasable and persistent; unsupported generated promises remain visible as disabled `FUTURE` cards. Thirteen former containerless capacity nodes are absent with migration refunds. The living Leader chooses at most one affordable supported study per rolling real-life day. Seventy-four maintained runtime recipes are authoritative. | verified | Quantitative category guards, supported dependency-order purchase, atomic future denial, exact 30 recipe/27 resource future count, migrations, signed server/SQLite behavior, deterministic Leader selection, descriptors, research UI/framebuffer, and strict gates |
...
| --- | --- | --- |
| `docs/GAME_VISION.md` | current intent, partially implemented | Seven-role ownership, exact controls, physical farm labor, all ten maintained processor routes, the truthful 487-node about-500 research catalog/purchase/persistence/client ledger, and the distinct larger communal blueprint are verified; finish unconsumed research payloads, sourced recipe/resource breadth, and authoritative shared-space/physical-trade depth |
...
| `p12-idle-cat-forest.md` | partial | Seven specialist officer roles plus the bounded Leader safety floor, role-building gates, signed manual work, shrine reachability, useful tools/costs, physical and wire-confidential Accountant reporting, all ten physical processors, Steward-managed local reserves, all 19 maintained skill gain/effect/UI paths, and physical Forester replanting/regrowth are verified; extend local inventories under the P19-canonical source contract |
...
| `p15-playtest-feedback.md` | partial | Roads, exact footprints/depth, coordinate placement, governance plus between-term election timing, useful-labor pressure, knowledge-blind shrine-return scouting in native and browser clients, the exact 48-hour baseline Leader safety-floor campaigns, and secure village foundations are verified; recipe/resource breadth remains |
| `p16-village-blueprint.md` | partial | The 15-adult/three-five-bed-Den lifecycle, canonical founding store mix, one logical 16px-art tile per authoritative grid cell, founding clearing, exterior water, exact roads/gate, selectable/removable gather controls, physical shoreline fishing, exterior agricultural territory, and research-free placement of Wood Cutter, Stone Prep, and Woodworking are verified. All retain open-top identities and now have P19-canonical physical input, selected queue, work, output, and final-delivery routes. Woodworking's Tool output becomes one stable finite identity under P19.C3. |
| `p17-biome-generator.md` | simulation-heavy, product partial | Fine-biome movement and physical rail/shipping are live; expose the remaining biome-specific resource ecology |
| `p18-visual-polish.md` | verified | Open stations, an integrated Accounting Tent, the label-free map, square top-down timber walls/exterior agriculture, the Adventure panel/button/progress/cursor skin, and unique semantic tracked glyphs for all 25 resource readouts are verified in native and optimized WASM campaigns; the exact 1024×768 semantic-HUD frame has no overlap or clipping |
| `p19-items-materials-trade.md` | canonical taxonomy, implementation partial | Stable source/station names, open-top stations, all ten processors, 74 selected physical recipes, complete food/plant and subsistence/frontier families, exact finite goods, functional equipment, scaffold delivery, wear/repair, and physical trade are live. Remaining work is the 30 recipe/27 resource future breadth in `RECIPE_RESOURCE_MATRIX.md` and deeper inventories. |
...
| Blessings, buildings, and the original ~18-node research tree | Carried forward and expanded: active shrine faucets, useful tools, escalating costs, supported research-point purchases, deterministic Leader-owned daily selection, persistence, and modeled effects are verified. Unsupported generated recipe/resource and thirteen unsafe/passive Crews promises are truthfully visible but non-purchasable; twelve Crews nodes add real stations. Durability affects repair, physical recipe studies gate 13 operations, and building placement is authoritative. Research labor/building automation and rituals remain Loremaster-owned. |
