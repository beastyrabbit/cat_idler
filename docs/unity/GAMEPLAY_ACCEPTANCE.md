...
Three scalar-haul cases verify transfers between existing stores, partial source reservations and destination changes. Selection excludes the source and considers only the unclaimed amount. Carriers keep their goods while blocked or while all other stores are full, then finish the same haul after an accepting destination becomes available. Public removal, designation and controlled-cat deposit/pickup actions exercise those changes without duplicating goods.
