...
The final September 6 simulation run passed 588 focused acceptance scenarios in 8.5028 seconds, including all 108 recipes, 487 research purchases, runtime effects, 86 regressions and the three shipping recovery scenarios. Earlier failing cases led to corrections for construction's handling of carried Food and surplus Planks, Armor storage, new-work cargo, and resumed roads, rails and expansion. The expansion case also checks every paid perimeter segment. Thirty territory cases verify planning and delayed work against foreign claims and physical footprints, safe founding, blocked recovery and permitted own/shared construction. Existing assertions were preserved.
...
Nine extended scenarios passed in 165.208 seconds after the final territory and imported-output fixes: fresh communal colonies for 48 hours, established communal colonies for 72 hours, and shared communal plus personal villages for 48 hours, each at seeds 7, 41 and 127. Each scenario compares a twin advanced in smaller partitions, checks invariants and tracks original founding identities so migrants cannot hide deaths. These workloads contain no transport routes or imported batch outputs. The focused, authority and Unity suites provide direct evidence for those paths. Two authority/import cases also verify that publicly staffing an empty imported queue recovers exact outputs with one or two workers, preserving identity and capacity through restart. The established campaign has one staffed repeating Wood Cutter and finite initial supplies; it does not establish indefinite operation of a mature economy.
