...
The Unity simulation uses explicit C# data and a single authority. Movement is kinematic on an authoritative tile route; Unity physics does not decide resource ownership or job completion. All actor movement, needs, work and transport advance in 0.05-second steps. Planning and ecology retain their slower schedules. Unchanged failed path searches wait until the next planning boundary, while changed destinations retry immediately. Third-person control and the management camera share this world.
...
Five scalar-haul cases verify transfers between existing stores, partial source reservations and destination changes. Selection excludes the source and considers only the unclaimed amount. Carriers keep their goods while blocked or while all other stores are full, then finish the same haul after an accepting destination becomes available. Public removal, designation and controlled-cat deposit/pickup actions exercise those changes without duplicating goods. A permanent avoid zone over the original receiving store forces a loaded carrier to choose another reachable store. Competing partial loads retain their capacity limits, and a separate real save/reload test resumes the saved input-delivery phase without changing carrier or job identity.
...
An earlier run of nine extended scenarios passed in 213.7647 seconds after the complete road and shore-placement corrections: fresh communal colonies for 48 hours, established communal colonies for 72 hours, and shared communal plus personal villages for 48 hours, each at seeds 7, 41 and 127. Each compares a twin advanced in smaller partitions, checks invariants and tracks original founding identities so migrants cannot hide deaths. All founding cats survived without a reset. This run used `61d29f1`, before the final farm-edge projection, crop-rendering and transport corrections. These workloads create no road, rail or expansion jobs, designated farms, transport routes, active village trades or imported batch outputs. The final 655-case run directly covers infrastructure guards, physical construction, blocked transport and interrupted resumption. Authority/import tests cover imported output delivery and restart separately. The established campaign has one staffed repeating Wood Cutter and finite initial supplies; it does not establish indefinite operation of a mature economy.

The [acceptance ledger](ACCEPTANCE.md) records the current live-simulation checks, native observations and campaign results. The preceding historical counts describe the checks performed for their respective corrections.
