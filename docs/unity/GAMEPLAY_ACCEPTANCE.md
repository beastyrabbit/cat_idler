...
Later construction requires an entrance on the existing shrine-connected road network. Disconnected paving and worn dirt do not qualify. Roads consume materials and physical work; placing a building creates no free access road. Construction cannot cover the shrine ring, existing entrances, or queued road and rail paths. Existing Unity and imported saves retain their shrine coordinates, footprints and gate convention. Compatibility reads imported `road_built` paths without relaying terrain or resetting progress.
...
| Roads and walls | Roads consume delivered material per completed tile. Pending road and rail paths reserve their footprints and recheck permanent obstacles before work. Interrupted routes retain job identity and suspended claims; loose cargo spills do not block their resumption. Expansion preserves paid wall segments, keeps the layout's gates open and clears obsolete interior walls. New layouts have four gates. Farms remain exterior. |
...
The final simulation run passed 605 noncampaign scenarios in 17.6016 seconds, including all 108 recipes, 487 research purchases, runtime effects, 103 regressions and the three shipping recovery scenarios. The regressions also passed separately in 3.1602 seconds. Earlier failures led to corrections for carried Food and surplus Planks, Armor storage, new-work cargo, and resumed infrastructure. Thirty territory cases cover foreign claims and physical footprints during planning, work, founding and recovery.
...
Nine extended scenarios passed in 221.0792 seconds after the founding and movement corrections: fresh communal colonies for 48 hours, established communal colonies for 72 hours, and shared communal plus personal villages for 48 hours, each at seeds 7, 41 and 127. Each compares a twin advanced in smaller partitions, checks invariants and tracks original founding identities so migrants cannot hide deaths. All founding cats survived without a reset. This run preceded the final pending-route guard and its spill exclusion; the workloads create no road/rail jobs, transport routes or imported batch outputs. The final 605-case run directly covers the route guard and interrupted-road resumption. Authority/import tests cover imported output delivery and restart separately. The established campaign has one staffed repeating Wood Cutter and finite initial supplies; it does not establish indefinite operation of a mature economy.
